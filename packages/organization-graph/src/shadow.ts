...
function allowedHostname(hostname: string, networkMode: 'loopback' | 'internal'): boolean {
  const unbracketed =
...
function normalizeShadowOrigin(value: string, networkMode: 'loopback' | 'internal'): string {
  if (typeof value !== 'string' || value.length === 0 || value.length > 2_048) {
    throw new TypeError('Graph shadow origin must be a non-empty bounded URL.');
  }
  let parsed: URL;
  try {
    parsed = new URL(value);
  } catch {
    throw new TypeError('Graph shadow origin must be an absolute URL.');
  }
  if (
    ...
    throw new TypeError(
      `Graph shadow origin must be an exact credential-free ${networkMode} HTTP origin.`,
    );
  }
