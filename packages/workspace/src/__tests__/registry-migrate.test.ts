...
describe('migrateV1ToV2', () => {
  it('converts v1 entry to v2 schema', () => {
    ...
    const result = migrateV1ToV2(data, 'test_agent');

    expect(result.schema).toBe('openslack.agent_registry.v2');
    expect(result.agent_id).toBe('test_agent');
  ...
  it('generates identity stub', () => {
    ...
    expect(result.identity.uid).toBe('test_agent');
    expect(result.identity.principal_id).toBe('principal:test_agent');
  ...
  it('defaults missing optional fields', () => {
    const result = migrateV1ToV2({ schema: 'openslack.agent_registry.v1' } as Record<string, unknown>, 'minimal');

    expect(result.permissions.paths.allow).toEqual(['**']);
    expect(result.permissions.paths.deny).toEqual([]);
    expect(result.permissions.max_risk_zone).toBe('yellow');
    expect(result.vendor.provider).toBeUndefined();
    expect(result.repositories.workspace_repo.owner).toBe('unknown');
  });

  it('sets default action permissions', () => {
    const result = migrateV1ToV2({ schema: 'openslack.agent_registry.v1' } as Record<string, unknown>, 'x');
...
describe('migrateRegistry', () => {
  ...
  it('reports already_v2 for v2 entries', () => {
    writeRegistry('v2_agent', `schema: openslack.agent_registry.v2\nagent_id: "v2_agent"\n`);
    const results = migrateRegistry(fixtureRoot);
    expect(results).toEqual([{ agentId: 'v2_agent', status: 'already_v2' }]);
  });

  it('converts v1 entries in preview mode', () => {
    writeRegistry('v1_agent', V1_YAML);
    const results = migrateRegistry(fixtureRoot, { apply: false });
    expect(results).toHaveLength(1);
    expect(results[0].status).toBe('converted');
    expect(results[0].agentId).toBe('v1_agent');
  });

  it('does not write in preview mode', () => {
    writeRegistry('v1_agent', V1_YAML);
    migrateRegistry(fixtureRoot, { apply: false });
    const content = readFileSync(join(registryDir(), 'v1_agent.yaml'), 'utf-8');
  ...
  it('backs up v1 file in apply mode', () => {
    writeRegistry('v1_agent', V1_YAML);
    migrateRegistry(fixtureRoot, { apply: true });
    const backupPath = join(fixtureRoot, '.openslack', 'agents', 'registry.v1-backup', 'v1_agent.yaml');
  ...
  it('reports error for unknown schema', () => {
    writeRegistry('weird', `schema: something.else\nagent_id: "weird"\n`);
    const results = migrateRegistry(fixtureRoot);
  ...
  it('handles mixed entries', () => {
    writeRegistry('v1_agent', V1_YAML);
    writeRegistry('v2_agent', `schema: openslack.agent_registry.v2\nagent_id: "v2_agent"\n`);
    writeRegistry('bad_agent', `schema: unknown.schema\n`);
    const results = migrateRegistry(fixtureRoot);
