...
describe('transactional workspace attach', () => {
  ...
  it('rolls back when generated bytes are changed before post-validation', () => {
    const root = gitRoot();
    const original = Buffer.from('# original\n', 'utf8');
    writeFileSync(join(root, '.gitignore'), original);
    const plan = attachPlan(root, 'read-only-monitor');
    ...
    ).toThrow(/bytes do not match the plan/u);
  ...
  it('keeps a validated attach committed when journal cleanup is deferred', () => {
    const root = gitRoot();
    const result = applyWorkspaceAttach(attachPlan(root, 'full-agent'), {
      hooks: {
    ...
    const beforeRecovery = snapshotGeneratedFiles(root);

    const recovered = applyWorkspaceAttach(attachPlan(root, 'full-agent'));

    expect(recovered.changed).toBe(false);
    expect(recovered.recoveredTransaction).toBe(true);
    expect(recovered.journalCleanupDeferred).toBe(false);
    expect(snapshotGeneratedFiles(root)).toEqual(beforeRecovery);
    expect(
      existsSync(join(root, '.openslack.local', 'transactions', 'attach.committed.json')),
    ).toBe(false);
  });
