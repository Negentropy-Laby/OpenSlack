...
export function decideMerge(input: MergeInput): MergeDecision {
  if (input.riskZone === 'black') {
    return {
      decision: 'deny',
      reason: 'Black Zone files touched — PR rejected automatically',
      riskZone: 'black',
    };
  }
  ...
  if (!input.validation || input.validation.decision === 'fail') {
    return {
      decision: 'deny',
      reason: 'Validation failed — see self_validation.yaml',
  ...
  if (input.riskZone === 'red' && !input.humanApproval) {
    return {
      ...
      reason: 'Red Zone requires human approval',
      riskZone: 'red',
    };
  }

  // Check independent review — implementation agent != review agent
  const selfReviewed = input.reviews.some((r) => r.reviewerAgent === r.implementationAgent);
  if (selfReviewed) {
    return {
      decision: 'wait',
      reason: 'Implementation agent cannot review own PR',
      riskZone: input.riskZone,
    };
  }
  ...
  if (!hasRequiredReviews && input.riskZone !== 'green') {
    return {
      decision: 'wait',
      reason: 'Independent agent review required',
      riskZone: input.riskZone,
    };
  }

  return {
    decision: 'merge_queue',
    reason: 'All gates passed — eligible for merge queue',
    riskZone: input.riskZone,
  };
}
