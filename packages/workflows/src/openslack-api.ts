...
import {
  recordEvent as collabRecordEvent,
  createHandoff as collabCreateHandoff,
  recordDecision as collabRecordDecision,
...
export interface OpenSlackAPIOptions {
  ...
  ) => Promise<MergeStewardCallResult>;
  /** Override listOpenPRs for testing */
  _listOpenPRs?: (
    limit?: number,
  ) => Promise<Array<{ number: number; title: string; status: string }>>;
...
export function createOpenSlackAPI(options: OpenSlackAPIOptions = {}) {
  ...
  const doRecordEvent: (event: unknown) => unknown =
    options._recordEvent ??
    ((event: unknown) => {
      collabRecordEvent(event as Parameters<typeof collabRecordEvent>[0]);
    });
  const doCreateHandoff: (details: unknown) => unknown =
    options._createHandoff ??
    ((details: unknown) =>
      collabCreateHandoff(details as Parameters<typeof collabCreateHandoff>[0]));
  const doRecordDecision: (details: unknown) => unknown =
    options._recordDecision ??
    ((details: unknown) =>
      collabRecordDecision(details as Parameters<typeof collabRecordDecision>[0]));
  ...
  return {
    task: {
      ...
      async sync(_issueNumber: number): Promise<{ pushed: boolean; prUrl?: string }> {
        return { pushed: false };
      },
    },
    ...
    prms: {
      ...
      async doctor(prNumber: number): Promise<PrmsDoctorResult> {
        try {
          ...
          const zone = diagnosed.riskZone === 'black' ? ('red' as const) : diagnosed.riskZone;

          const status: PrmsDoctorResult['status'] = isReady ? 'READY_TO_MERGE' : 'BLOCKED';
          ...
          return {
            status: 'ERROR',
            blockers: [
              { gate: 'system', reason: err instanceof Error ? err.message : String(err) },
            ],
            zone: 'red',
       ...
       */
      async requestMerge(prNumber: number): Promise<{ merged: boolean; prmsStatus: string }> {
        const policy = doLoadPolicy();
