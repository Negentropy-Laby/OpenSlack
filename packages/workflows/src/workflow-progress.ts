...
async function readJsonl<T>(path: string, label: string): Promise<{ values: T[]; warning?: string }> {
  try {
...
function readEvidence(result: AgentResult | Record<string, unknown>, filename: string): WorkflowAgentProgress {
  const agentResult = result as AgentResult
  const evidence = agentResult.workflowEvidence
  const tokenUsage = typeof agentResult.tokenUsage === 'number'
    ? agentResult.tokenUsage
    : typeof evidence?.tokenUsage === 'number'
      ? evidence.tokenUsage
      : 0
  const agentRunId = typeof agentResult.runId === 'string' ? agentResult.runId : evidence?.agentRunId
  return {
...
function toolEvidenceFromTranscript(events: AgentRunEvent[]): WorkflowToolEvidence[] {
  return events
    .filter((event) => event.type === 'tool_call' || event.type === 'tool_result' || event.type === 'progress')
    .slice(-8)
    .map((event) => {
...
async function readAgentResults(runDir: string, rootDir: string, warnings: string[]): Promise<WorkflowAgentProgress[]> {
  ...
  for (const file of files.filter((entry) => entry.endsWith('.json'))) {
    const read = await readJson<AgentResult | Record<string, unknown>>(join(agentDir, file), `agent result ${file}`)
...
function groupPhases(
  ...
  return [...phaseNames].map((phase) => {
    ...
    return {
      phase,
      status: checkpoint ? normalizePhaseStatus(checkpoint.status) : status?.currentPhase === phase ? 'running' : 'not-started',
...
async function loadWorkflowMeta(rootDir: string, workflowName?: string): Promise<WorkflowMeta | null> {
...
export async function getWorkflowRunProgress(
  ...
  const pendingRead = await readJson<PendingApproval[]>(join(runDir, 'pending-approvals.json'), 'pending approvals')
  if (pendingRead.warning && pendingRead.warning !== 'pending approvals not recorded') warnings.push(pendingRead.warning)
  const logRead = await readJsonl<ProgressLogEntry>(join(runDir, 'log.jsonl'), 'workflow log')
  if (logRead.warning && logRead.warning !== 'workflow log not recorded') warnings.push(logRead.warning)
  const outputRead = await readJson<unknown>(join(runDir, 'output.json'), 'workflow output')
  if (outputRead.warning && outputRead.warning !== 'workflow output not recorded') warnings.push(outputRead.warning)
  ...
  return {
    ...
    pendingApprovalCount: (pendingRead.value ?? []).filter((approval) => approval.status === 'pending').length,
    budget,
    phases,
    outputSummary: outputRead.value === null ? undefined : summarize(outputRead.value),
    logTail: logRead.values.slice(-8).map((entry) => `${entry.ts} ${entry.phase ? `[${entry.phase}] ` : ''}${redactString(entry.message)}`),
    warnings,
  }
}
...
export function renderWorkflowRunProgress(progress: WorkflowRunProgress): string {
  ...
  for (const phase of progress.phases) {
    lines.push(`  - ${phase.phase}: ${phase.status}, agents ${phase.agentCount}, tokens ${phase.tokenTotal}`)
    for (const agent of phase.agents) {
      lines.push(`      ${agent.label}: ${agent.status}, model ${agent.model ?? 'not recorded'}, isolation ${agent.isolation ?? 'not recorded'}, tokens ${agent.tokensUsed}`)
      ...
      for (const tool of agent.recentTools.slice(-3)) lines.push(`        tool: ${tool.name} (${tool.type}) ${tool.summary}`)
    }
  }
