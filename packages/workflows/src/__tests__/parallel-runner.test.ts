...
describe('runParallel', () => {
  ...
  it('respects concurrency limit', async () => {
    const order: number[] = []
    const tasks = [
      async () => { order.push(0); await delay(10); return 'a' },
      async () => { order.push(1); await delay(10); return 'b' },
      async () => { order.push(2); await delay(10); return 'c' },
      async () => { order.push(3); await delay(10); return 'd' },
    ]
  ...
  it('throws when budget is exhausted', async () => {
    ...
    await expect(
      runParallel([async () => 1], undefined, budget),
