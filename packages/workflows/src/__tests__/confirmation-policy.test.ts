...
function makePolicy(
  manifest?: WorkflowApprovalManifest,
...
describe('validateEffectAgainstManifest', () => {
  it('allows approved effects', () => {
    const manifest = makeManifest([
      { operation: 'openslack.task.createIssue', detail: 'Create issue' },
    ]);
    const policy = makePolicy(manifest);
    const result = validateEffectAgainstManifest(
...
describe('createOnConfirmFromPolicy', () => {
  ...
  it('throws WorkflowPausedError for unapproved effects when onUnexpectedEffect is pause', async () => {
    ...
    await expect(onConfirm('openslack.task.checkout', 'Checkout')).rejects.toThrow(
      WorkflowPausedError,
    );
  });
