...
describe('validateAgainstSchema', () => {
  ...
  it('returns violation for enum mismatch', () => {
    const violations = validateAgainstSchema('invalid', { enum: ['valid', 'also-valid'] });
    expect(violations.length).toBeGreaterThan(0);
  });

  it('validates array items', () => {
    const violations = validateAgainstSchema([1, 'bad'], {
      type: 'array',
      items: { type: 'number' },
    });
    expect(violations.length).toBeGreaterThan(0);
...
describe('executeAgentCall', () => {
  ...
  it('redacts credential-shaped prompt text before provider preflight and execution', async () => {
    ...
    await executeAgentCall(
      `inspect without exposing ${fakeToken}`,
      { label: 'test', phase: 'Scan' },
      config,
    );
    expect(launcher.preflight).toHaveBeenCalledWith(
      expect.not.stringContaining(fakeToken),
      expect.any(Object),
    );
  ...
  it('charges reported provider usage when execution fails after a response', async () => {
    ...
    const { config, budget } = makeConfig({
      launcher,
      budget: { tokensUsed: 2, tokensRemaining: 10, costUsd: 0, agentCalls: 0 },
    });
    await expect(
      executeAgentCall('prompt', { label: 'test', phase: 'Scan' }, config),
  ...
  it('bounds the launcher request by workflow and per-call token budgets', async () => {
    const launcher = vi.fn(async () => ({ data: { ok: true }, tokenUsage: 1 }));
    const { config } = makeConfig({
      launcher: launcher as AgentLauncher,
      budget: { tokensUsed: 10, tokensRemaining: 60, costUsd: 0, agentCalls: 0 },
    });
    await executeAgentCall(
      'prompt',
      { label: 'test', phase: 'Scan', budget: { tokens: 100 } },
      config,
    );
    expect(launcher).toHaveBeenCalledWith(
      'prompt',
      expect.objectContaining({ budget: { tokens: 60, costUsd: undefined } }),
    );
  });
  ...
  it('does not emit completed lifecycle evidence for an invalid result schema', async () => {
    ...
    expect(eventEmitter).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'agent.conversation.started' }),
    );
    expect(eventEmitter).not.toHaveBeenCalledWith(
      expect.objectContaining({ type: 'agent.conversation.completed' }),
    );
    expect(eventEmitter).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'agent.conversation.failed' }),
    );
  });
