...
describe('resolveAgentType', () => {
  ...
  it('returns config for valid agentType in OpenSlack registry', () => {
    ...
    try {
      ...
      writeFileSync(join(registryDir, 'test-agent.yaml'), [
  ...
  it('returns config for valid agentType in .claude/agents/ (project-level)', () => {
    ...
    try {
      ...
      writeFileSync(join(agentsDir, 'my-subagent.md'), [
        '---',
        'name: My Subagent',
  ...
  it('prefers OpenSlack registry over .claude/agents/', () => {
    ...
    try {
      ...
      const registryDir = join(root, '.openslack', 'agents', 'registry')
      mkdirSync(registryDir, { recursive: true })
      writeFileSync(join(registryDir, 'shared-agent.yaml'), [
        'schema: openslack.agent_registry.v1',
      ...
      const agentsDir = join(root, '.claude', 'agents')
      mkdirSync(agentsDir, { recursive: true })
      writeFileSync(join(agentsDir, 'shared-agent.md'), [
        '---',
  ...
  it('returns model as undefined when registry model is "default"', () => {
    ...
    try {
      const registryDir = join(root, '.openslack', 'agents', 'registry')
      mkdirSync(registryDir, { recursive: true })
      writeFileSync(join(registryDir, 'default-model.yaml'), [
        'schema: openslack.agent_registry.v1',
  ...
  it('maps subagent isolation field to config', () => {
    ...
    try {
      const agentsDir = join(root, '.claude', 'agents')
      mkdirSync(agentsDir, { recursive: true })
      writeFileSync(join(agentsDir, 'isolated-agent.md'), [
        '---',
        'name: Isolated Agent',
  ...
  it('caches subagent discovery results within TTL', () => {
    ...
    try {
      const agentsDir = join(root, '.claude', 'agents')
      mkdirSync(agentsDir, { recursive: true })
      writeFileSync(join(agentsDir, 'cached-agent.md'), [
        '---',
        'name: Cached Agent',
      ...
      writeFileSync(join(agentsDir, 'after-cache.md'), [
        '---',
        'name: After Cache',
  ...
  it('bypasses cache when rootDir changes', () => {
    ...
    try {
      ...
      writeFileSync(join(dirA, 'agent-a.md'), [
        '---',
        'name: Agent A',
        'description: Agent in root A',
        '---',
        'Prompt A.',
      ].join('\n'))
      ...
      writeFileSync(join(dirB, 'agent-b.md'), [
        '---',
        'name: Agent B',
        'description: Agent in root B',
        '---',
        'Prompt B.',
      ].join('\n'))
