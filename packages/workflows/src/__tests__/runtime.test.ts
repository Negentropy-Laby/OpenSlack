...
describe('createRuntime', () => {
  ...
  describe('agent', () => {
    ...
    it('throws in validate mode', async () => {
      ...
      await expect(rt.agent('prompt', { label: 'test', phase: 'Scan' })).rejects.toThrow(
        'validate mode',
      );
    });
  ...
  describe('parallel', () => {
    ...
    it('executes tasks with results in order', async () => {
      const rt = makeRuntime();
      const result = await rt.parallel([async () => 'a', async () => 'b', async () => 'c']);
      expect(result).toEqual(['a', 'b', 'c']);
    });

    it('respects concurrency option', async () => {
      const rt = makeRuntime();
      const result = await rt.parallel([async () => 1, async () => 2, async () => 3], {
        concurrency: 2,
      });
  ...
  describe('workflow', () => {
    ...
    it('exposes dynamic workflow helpers without breaking child workflow calls', async () => {
      ...
      const route = rt.workflow.routeModelAndIsolation({
        label: 'verify',
  ...
  describe('preview mode openslack restrictions', () => {
    it('blocks task.createIssue in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.task.createIssue({ title: 'Bug' })).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks task.checkout in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.task.checkout(42, 'agent-1')).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks prms.requestMerge in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.prms.requestMerge(1)).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks collaboration.recordEvent in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.collaboration.recordEvent({ type: 'test' })).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks collaboration.createHandoff in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.collaboration.createHandoff({})).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks collaboration.recordDecision in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.collaboration.recordDecision({})).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
    ...
    it('blocks governance.audit in preview mode', async () => {
      const rt = makeRuntime({ mode: 'preview' });
      await expect(rt.openslack.governance.audit('test')).rejects.toThrow(
        'not allowed in preview mode',
      );
    });
