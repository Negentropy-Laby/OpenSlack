...
function normalize(raw: unknown, source: WorkflowDisablePolicy['source']): WorkflowDisablePolicy {
  ...
  const workflows =
    obj.workflows && typeof obj.workflows === 'object'
      ? (obj.workflows as Record<string, unknown>)
      : obj;
  return {
    enabled: typeof workflows.enabled === 'boolean' ? workflows.enabled : DEFAULT_POLICY.enabled,
    ultracode:
      typeof workflows.ultracode === 'boolean' ? workflows.ultracode : DEFAULT_POLICY.ultracode,
    maxConcurrency:
      typeof workflows.max_concurrency === 'number'
        ? workflows.max_concurrency
        : DEFAULT_POLICY.maxConcurrency,
    maxAgentsPerRun:
      typeof workflows.max_agents_per_run === 'number'
        ? workflows.max_agents_per_run
        : DEFAULT_POLICY.maxAgentsPerRun,
    source,
...
export function readWorkflowPolicy(options: WorkflowPolicyOptions = {}): WorkflowDisablePolicy {
  if (process.env.OPENSLACK_DISABLE_WORKFLOWS === '1') {
    return {
      ...
      enabled: false,
      source: 'env',
      reason: 'OPENSLACK_DISABLE_WORKFLOWS=1',
    };
  }
