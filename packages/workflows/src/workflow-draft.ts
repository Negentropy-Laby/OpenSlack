...
function createManifest(
  prompt: string,
  pattern: WorkflowPatternManifest,
  createdAt: string,
): WorkflowMeta {
...
export async function generateWorkflowDraft(
  options: GenerateWorkflowDraftOptions,
...
export async function previewWorkflowDraft(
  options: PreviewWorkflowDraftOptions,
): Promise<WorkflowDraftPreview> {
  const rootDir = options.rootDir ?? process.cwd();
  const path =
    options.draftIdOrPath.includes('/') || options.draftIdOrPath.includes('\\')
  ...
  return {
    ...
    budgetEstimate: loaded.meta.budgetPolicy ?? {
      tokenBudget: 100000,
      maxAgents: 1000,
      maxConcurrency: 16,
      onExceeded: 'pause',
...
export function renderWorkflowDraftPreview(preview: WorkflowDraftPreview): string {
  ...
  lines.push(
    `Budget: ${preview.budgetEstimate.tokenBudget ?? 'unlimited'} tokens, max agents ${preview.budgetEstimate.maxAgents ?? 'default'}, concurrency ${preview.budgetEstimate.maxConcurrency ?? 'default'}`,
  );
  lines.push('');
  lines.push('Phases:');
  ...
  lines.push('');
  lines.push(
    `Side effects: ${preview.sideEffects.length > 0 ? preview.sideEffects.join(', ') : 'none declared'}`,
  );
  return lines.join('\n');
}
