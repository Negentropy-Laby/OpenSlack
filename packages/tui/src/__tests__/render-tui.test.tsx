...
describe('renderTui', () => {
  ...
  it('renders themed component and returns unmount', async () => {
    ...
    const { unmount } = await renderTui(React.createElement(Text, null, 'hello tui'), {
  ...
  it('throws when isTuiSupported is false and no stdout override', async () => {
    await expect(renderTui(React.createElement(Text, null, 'test'))).rejects.toThrow(
      'TUI is not supported',
    );
  });
  ...
  it('enables mouse tracking immediately after alt-screen render starts', async () => {
    ...
    const { unmount } = await renderTui(React.createElement(Text, null, 'mouse ready'), {
      stdout: stdout as unknown as NodeJS.WriteStream,
    });
    unmountFn = () => {
      setIsTTY(false);
  ...
  it('setAltScreenActive(true, true) writes mouse tracking immediately', async () => {
    const { stdout, chunks, setIsTTY } = createMockStdout({ isTTY: true });
    const instance = await renderInk(React.createElement(Text, null, 'direct enable'), {
      stdout: stdout as unknown as NodeJS.WriteStream,
    });
    unmountFn = () => {
      setIsTTY(false);
  ...
  it('setAltScreenActive(false) disables mouse tracking after it was enabled', async () => {
    const { stdout, chunks, setIsTTY } = createMockStdout({ isTTY: true });
    const instance = await renderInk(React.createElement(Text, null, 'direct disable'), {
      stdout: stdout as unknown as NodeJS.WriteStream,
    });
    unmountFn = () => {
      setIsTTY(false);
  ...
  it('setAltScreenActive(true, false) does not write mouse tracking', async () => {
    const { stdout, chunks, setIsTTY } = createMockStdout({ isTTY: true });
    const instance = await renderInk(React.createElement(Text, null, 'no mouse'), {
      stdout: stdout as unknown as NodeJS.WriteStream,
    });
    unmountFn = () => {
      setIsTTY(false);
