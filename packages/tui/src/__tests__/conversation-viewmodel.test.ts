...
function makeMessage(
  ...
  switch (kind) {
    case 'user_message':
      return { ...base, kind: 'user_message', text: 'Hello world', ...overrides } as AgentConversationMessage
    case 'agent_response':
      return { ...base, kind: 'agent_response', text: 'Agent reply', ...overrides } as AgentConversationMessage
    case 'tool_event':
      return { ...base, kind: 'tool_event', toolName: 'read_file', ...overrides } as AgentConversationMessage
    case 'plan':
      return { ...base, kind: 'plan', planId: 'PLAN-001', steps: ['Step 1', 'Step 2'], ...overrides } as AgentConversationMessage
    case 'approval_request':
      return { ...base, kind: 'approval_request', targetAction: 'deploy', riskLevel: 'high', ...overrides } as AgentConversationMessage
    case 'decision':
      return { ...base, kind: 'decision', decisionId: 'DEC-001', summary: 'Decided to proceed', ...overrides } as AgentConversationMessage
    case 'handoff':
      return { ...base, kind: 'handoff', handoffId: 'HO-001', toParticipant: 'agent-2', summary: 'Handing off', ...overrides } as AgentConversationMessage
  }
}
...
describe('mapConversationListToViewModel', () => {
  it('maps threads correctly', () => {
    const threads = [
      ...
      makeThread({ id: 'THREAD-002', title: 'Second Thread', status: 'completed', updatedAt: new Date(Date.now() - 60000).toISOString() }),
    ]
...
describe('mapThreadToViewModel', () => {
  ...
  it('sorts messages chronologically', () => {
    const thread = makeThread()
    const messages = [
      makeMessage('user_message', { id: 'MSG-2', timestamp: new Date(Date.now() - 1000).toISOString() }),
      makeMessage('user_message', { id: 'MSG-1', timestamp: new Date(Date.now() - 2000).toISOString() }),
    ]
    const model = mapThreadToViewModel(thread, messages)
