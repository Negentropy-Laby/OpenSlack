...
function assertNoLineExceeds(output: string, maxWidth: number) {
  ...
  for (const line of lines) {
    ...
    expect(
      visualWidth,
      `Line exceeds ${maxWidth} columns (${visualWidth}): "${line}"`,
...
describe('renderPlainProfile', () => {
  ...
  it('renders guided flow at check step', () => {
    const vm: ProfileViewModel = {
      ...
      syncStatus: 'never',
      guidedStep: 'check',
      checkGroups: [{ key: 'source', label: 'Source', status: 'warn', detail: 'No commits yet' }],
    };
    const out = renderPlainProfile(vm);
  ...
  it('renders blocked gate items', () => {
    const vm: WorkflowLifecycleViewModel = {
      ...
      blockedGateItems: [
        ...
        {
          gate: 'Review',
          detail: 'No CODEOWNER approval',
          action: 'Request review from team-lead',
        },
      ],
    };
    const out = renderPlainWorkflowLifecycle(vm);
...
describe('renderPlainWorkflowWorkbench', () => {
  it('contains expected markers', () => {
    ...
    expect(out).toContain('[2] Watch');
    expect(out).toContain('[3] Approve');
    expect(out).toContain('[4] Reuse');
    expect(out).toContain('[5] Publish');
...
describe('renderPlain dispatch', () => {
  it('dispatches to correct renderer', () => {
    ...
    expect(renderPlain('workflow-lifecycle', createWorkflowLifecycleViewModel())).toContain(
      'Workflow Lifecycle',
    );
    expect(renderPlain('workflow-workbench', createWorkflowWorkbenchViewModel())).toContain(
      'Workflow Workbench',
    );
    expect(renderPlain('dashboard', createDashboardViewModel())).toContain(
      'OpenSlack Team Dashboard',
    );
    ...
    expect(renderPlain('decision-detail', createDecisionDetailViewModel())).toContain(
      'Decision: Adopt new branching strategy',
    );
    ...
    expect(renderPlain('handoff-detail', createHandoffDetailViewModel())).toContain(
      'Handoff: h-001',
    );
    ...
    expect(renderPlain('workflow-preview', createWorkflowPreviewViewModel())).toContain(
      ...
      attentionItems: [
        {
          ...
          detail:
            'https://github.com/Negentropy-Laby/OpenSlack/pull/130/files#diff-abc123def456ghi789jkl012',
          route: 'approvals',
          colorTheme: 'warning',
        },
      ],
  ...
  it('wraps next recommended action within the requested width', () => {
    const vm: HomeViewModel = {
      ...
      nextRecommendedAction: {
        label:
          'Approve profile sync PR https://github.com/Negentropy-Laby/OpenSlack/pull/130/files#diff-abc123def456',
        reason:
          'Validation is waiting at https://github.com/Negentropy-Laby/OpenSlack/actions/runs/26736208952/job/78789924481',
        route: 'approvals',
        urgency: 'governance',
        priority: 0,
      },
...
describe('CJK preservation across all renderers', () => {
  ...
  it('preserves CJK in issues/PRs view', () => {
    const vm: IssuesPrViewModel = {
      ...createIssuesPrViewModel(),
      issues: [{ number: 99, title: '인증 흐름 수정', status: 'ready', labels: ['버그'] }],
    };
