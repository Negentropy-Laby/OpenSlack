...
function makePreview(overrides?: Partial<WorkflowPreview>): WorkflowPreview {
  return {
    ...
    steps: [
      { phase: 'Build', type: 'action', title: 'Build project', actionId: 'build', sideEffects: false, requiresConfirmation: false },
      { phase: 'Deploy', type: 'action', title: 'Deploy to staging', actionId: 'deploy', sideEffects: true, requiresConfirmation: true, requiredRole: 'admin' },
    ],
    errors: [],
    ...overrides,
  }
...
describe('mapWorkflowPreviewToViewModel', () => {
  ...
  it('deduplicates phases', () => {
    const preview = makePreview({
      steps: [
        { phase: 'Build', type: 'action', title: 'Step 1', actionId: 'a', sideEffects: false, requiresConfirmation: false },
        { phase: 'Build', type: 'action', title: 'Step 2', actionId: 'b', sideEffects: false, requiresConfirmation: false },
        { phase: 'Test', type: 'action', title: 'Step 3', actionId: 'c', sideEffects: false, requiresConfirmation: false },
      ],
    })
  ...
  it('computes hasSideEffects correctly', () => {
    ...
    const model2 = mapWorkflowPreviewToViewModel(makePreview({
      steps: [
        { phase: 'Plan', type: 'action', title: 'Plan', actionId: 'plan', sideEffects: false, requiresConfirmation: false },
      ],
    }))
  ...
  it('sanitizes text fields', () => {
    const preview = makePreview({
      ...
      steps: [
        { phase: 'Phase\x1b[0m', type: 'action', title: 'Title\x0b', actionId: 'act\x1b[31m', sideEffects: false, requiresConfirmation: false },
      ],
  ...
  it('handles steps without actionId', () => {
    const preview = makePreview({
      steps: [
        { phase: 'Review', type: 'decision-gate', title: 'Approval gate', sideEffects: false, requiresConfirmation: true, requiredRole: 'reviewer' },
      ],
    })
    const model = mapWorkflowPreviewToViewModel(preview)
  ...
  it('handles steps without requiredRole', () => {
    const preview = makePreview({
      steps: [
        { phase: 'Run', type: 'action', title: 'Run task', actionId: 'run', sideEffects: true, requiresConfirmation: false },
      ],
    })
    const model = mapWorkflowPreviewToViewModel(preview)
