...
function createCjkActivityViewModel(): ActivityViewModel {
  return {
    ...
    totalEvents: 5,
    events: [
      { time: '12:00', type: `${ANSI_BOLD}pr.merged`, summary: `${CJK_JAPANESE_LABEL} PR merged ${EMOJI_TITLE}`, actor: `${CJK_KOREAN_LABEL}-agent`, objectKind: 'pr', objectId: '42', nextAction: LONG_URL, risk: 'high' },
      { time: '11:30', type: `check.${ANSI_COLORED}failed`, summary: `${CJK_CHINESE_LABEL} CI ${CJK_WITH_ANSI}`, actor: `${CJK_JAPANESE_LABEL}-bot`, objectKind: 'check', objectId: 'build' },
      { time: '11:00', type: 'task.completed', summary: LONG_URL, actor: `${CJK_KOREAN_LABEL}-agent`, objectKind: 'task', objectId: 'issue-15' },
      { time: '10:00', type: `${MIXED_CONTENT}workflow.run`, summary: `${OSC_HYPERLINK} ${CJK_CHINESE_LABEL}`, actor: `${ANSI_BOLD}-runner`, objectKind: 'workflow', objectId: 'deploy' },
      { time: '09:00', type: 'handoff.created', summary: `${CJK_KOREAN_LABEL} handoff ${CJK_JAPANESE_LABEL}`, actor: `${CJK_CHINESE_LABEL}-agent`, objectKind: 'handoff', objectId: 'h-001', owner: `${ANSI_COLORED}-lead` },
    ],
    today: [
      { time: '12:00', type: 'pr.merged', summary: `${CJK_JAPANESE_LABEL} PR merged ${EMOJI_TITLE}`, actor: `${CJK_KOREAN_LABEL}-agent`, objectKind: 'pr', objectId: '42' },
      { time: '11:30', type: 'check.failed', summary: `${CJK_CHINESE_LABEL} CI ${CJK_WITH_ANSI}`, actor: `${CJK_JAPANESE_LABEL}-bot`, objectKind: 'check', objectId: 'build' },
    ],
    yesterday: [
      { time: '09:00', type: 'handoff.created', summary: `${CJK_KOREAN_LABEL} handoff`, actor: `${CJK_CHINESE_LABEL}-agent`, objectKind: 'handoff', objectId: 'h-001' },
    ],
    older: [
      { time: '08:00', type: 'task.completed', summary: LONG_URL, actor: `${CJK_KOREAN_LABEL}-agent`, objectKind: 'task', objectId: 'issue-10' },
    ],
  }
}
...
function createCjkDecisionListViewModel(): DecisionListViewModel {
  return {
    ...
    activeCount: 1,
    items: [
      { id: `${ANSI_BOLD}-d-001`, topic: `${CJK_JAPANESE_LABEL}: ${EMOJI_TITLE} architecture`, decision: `${CJK_KOREAN_LABEL} monolith ${LONG_URL}`, status: 'active', decidedBy: `${ANSI_COLORED}-lead`, age: '2h' },
      { id: `${CJK_WITH_ANSI}-d-002`, topic: `${OSC_HYPERLINK} deploy strategy`, decision: `${CJK_CHINESE_LABEL} rolling ${MIXED_CONTENT}`, status: 'superseded', decidedBy: `${CJK_JAPANESE_LABEL}-architect`, age: '5d' },
    ],
  }
}
...
function createCjkDecisionDetailViewModel(): DecisionDetailViewModel {
  return {
    ...
    alternatives: [`${CJK_JAPANESE_LABEL} microservices ${EMOJI_TITLE}`, `${ANSI_BOLD} serverless ${CJK_KOREAN_LABEL}`],
    consequences: [`${CJK_WITH_ANSI} higher latency ${LONG_URL}`, `${CJK_CHINESE_LABEL} easier debugging ${EMOJI_TITLE}`],
    decidedBy: `${OSC_HYPERLINK}-lead`,
    createdAt: '2026-05-31T10:00:00Z',
    status: 'active',
    supersededBy: `${ANSI_COLORED}-d-003`,
    supersededAt: '2026-06-01T08:00:00Z',
    tags: [`${CJK_CHINESE_LABEL}arch`, `${ANSI_BOLD}priority`, `${CJK_JAPANESE_LABEL}v2`, EMOJI_TITLE],
  }
}
    ...
    groups: [
      {
        ...
        status: 'pass',
        events: [
          { time: '12:00', type: `${ANSI_BOLD}pr.merged`, summary: `${CJK_KOREAN_LABEL} PR merged ${LONG_URL}`, objectKind: 'pr', objectId: '42' },
          { time: '11:00', type: 'task.completed', summary: `${CJK_CHINESE_LABEL} task done ${CJK_WITH_ANSI}`, objectKind: 'task', objectId: 'issue-15' },
        ],
      },
      {
        ...
        status: 'fail',
        events: [
          { time: '10:00', type: 'check.failed', summary: `${MIXED_CONTENT} CI ${CJK_JAPANESE_LABEL}`, objectKind: 'check', objectId: 'build' },
        ],
      },
      {
        ...
        status: 'info',
        events: [
          { time: '09:00', type: 'workflow.run', summary: `${ANSI_BOLD} ${CJK_KOREAN_LABEL} workflow ${EMOJI_TITLE}`, objectKind: 'workflow', objectId: 'deploy' },
        ],
      },
    ],
    recommendedNext: [
      { objectKind: `${CJK_JAPANESE_LABEL}`, objectId: `${ANSI_COLORED}-42`, action: `${CJK_KOREAN_LABEL} review ${LONG_URL} ${EMOJI_TITLE}` },
      { objectKind: 'check', objectId: `${CJK_WITH_ANSI}-build`, action: `${ANSI_BOLD} re-run ${CJK_CHINESE_LABEL} CI` },
    ],
  }
}
...
function createCjkHandoffListViewModel(): HandoffListViewModel {
  return {
    ...
    openCount: 1,
    items: [
      { id: `${ANSI_BOLD}-h-001`, from: `${CJK_JAPANESE_LABEL}-agent`, to: `${CJK_KOREAN_LABEL}-agent`, status: 'open', context: `${MIXED_CONTENT} ${LONG_URL} ${CJK_CHINESE_LABEL}`, age: '2h', ref: `issue:${ANSI_COLORED}-15` },
      { id: `${CJK_WITH_ANSI}-h-002`, from: `${CJK_CHINESE_LABEL}-agent`, to: `${OSC_HYPERLINK}-agent`, status: 'closed', context: `${ANSI_BOLD} ${CJK_JAPANESE_LABEL} complete ${EMOJI_TITLE}`, age: '3d', ref: 'pr:42' },
    ],
  }
}
...
function createCjkHandoffDetailViewModel(): HandoffDetailViewModel {
  return {
    ...
    nextSteps: [`${CJK_JAPANESE_LABEL} review PR ${EMOJI_TITLE}`, `${ANSI_BOLD} merge ${CJK_KOREAN_LABEL}`, `${CJK_WITH_ANSI} close issue`],
...
function createCjkIssuesPrViewModel(): IssuesPrViewModel {
  return {
    tab: 'issues',
    issues: [
      { number: 15, title: `${ANSI_BOLD} ${CJK_CHINESE_LABEL} bug fix ${EMOJI_TITLE}`, status: 'ready', assignee: `${CJK_JAPANESE_LABEL}-agent`, labels: [`${CJK_KOREAN_LABEL}bug`, `${ANSI_COLORED}priority`] },
      { number: 16, title: `${CJK_WITH_ANSI} feature ${LONG_URL}`, status: 'blocked', assignee: undefined, labels: [`${OSC_HYPERLINK}help`] },
    ],
    prs: [
      { number: 42, title: `${ANSI_COLORED} ${CJK_JAPANESE_LABEL} deploy ${EMOJI_TITLE}`, status: 'ready', author: `${CJK_KOREAN_LABEL}-agent`, riskZone: `${ANSI_BOLD}green`, blocker: undefined, nextAction: MIXED_CONTENT },
      { number: 43, title: `${CJK_CHINESE_LABEL} refactor ${LONG_URL}`, status: 'blocked', author: `${CJK_WITH_ANSI}-bot`, riskZone: 'yellow', blocker: `${CJK_JAPANESE_LABEL} CI failed ${ANSI_COLORED}` },
    ],
    summary: {
...
function createCjkSetupViewModel(): SetupViewModel {
  return {
    ...
    passedChecks: 2,
    fixable: [
      { id: `${ANSI_BOLD}-fix-001`, title: `${CJK_KOREAN_LABEL} GitHub token ${EMOJI_TITLE}`, status: 'WARN', detail: LONG_URL, nextAction: `${CJK_CHINESE_LABEL} run setup`, command: `${ANSI_COLORED} openslack setup ${CJK_JAPANESE_LABEL}` },
    ],
    needsAction: [
      { id: `${CJK_WITH_ANSI}-act-001`, title: `${CJK_JAPANESE_LABEL} branch protection ${EMOJI_TITLE}`, status: 'FAIL', detail: `${MIXED_CONTENT} missing rules`, nextAction: `${CJK_KOREAN_LABEL} configure GitHub`, command: '' },
    ],
    ok: [
      { id: 'ok-001', title: `${CJK_CHINESE_LABEL} workspace valid ${EMOJI_TITLE}`, status: 'PASS', detail: `${CJK_JAPANESE_LABEL} all good ${ANSI_BOLD}`, nextAction: '', command: '' },
      { id: 'ok-002', title: `${ANSI_COLORED} modules ${CJK_KOREAN_LABEL}`, status: 'PASS', detail: `${OSC_HYPERLINK} found`, nextAction: '', command: '' },
    ],
  }
}
...
function createCjkWorkflowPreviewViewModel(): WorkflowPreviewViewModel {
  return {
    ...
    steps: [
      { phase: `${CJK_CHINESE_LABEL} build ${EMOJI_TITLE}`, type: 'build', title: `${ANSI_BOLD} ${CJK_JAPANESE_LABEL} compile ${LONG_URL}`, actionId: `${CJK_KOREAN_LABEL}-build`, sideEffects: false, requiresConfirmation: false, requiredRole: '' },
      { phase: `${CJK_CHINESE_LABEL} build ${EMOJI_TITLE}`, type: 'test', title: `${CJK_KOREAN_LABEL} test ${ANSI_COLORED}`, actionId: `${CJK_JAPANESE_LABEL}-test`, sideEffects: false, requiresConfirmation: false, requiredRole: `${CJK_WITH_ANSI}dev` },
      { phase: `${CJK_JAPANESE_LABEL} deploy 🔧`, type: 'deploy', title: `${EMOJI_TITLE} deploy ${MIXED_CONTENT}`, actionId: `${CJK_CHINESE_LABEL}-deploy`, sideEffects: true, requiresConfirmation: true, requiredRole: `${ANSI_BOLD}admin` },
    ],
    ...
    errors: [`${CJK_KOREAN_LABEL} validation ${ANSI_COLORED} ${LONG_URL}`, `${CJK_WITH_ANSI} ${CJK_JAPANESE_LABEL} error ${EMOJI_TITLE}`],
    hasErrors: true,
  }
}
