...
describe('terminal restore on unmount', () => {
  ...
  it('enable-then-disable mouse tracking writes disable sequence', async () => {
    ...
    instance = await renderInk(React.createElement(Text, null, 'mouse-pair'), {
      stdout: mock.stdout,
    });
  ...
  it('unmount after render completes without error', async () => {
    ...
    instance = await renderInk(React.createElement(Text, null, 'clean-exit'), {
      stdout: mock.stdout,
    });
  ...
  it('unmount after alt-screen enable completes without error', async () => {
    ...
    instance = await renderInk(React.createElement(Text, null, 'alt-exit'), {
      stdout: mock.stdout,
    });
    instance.setAltScreenActive(true, true);
    await new Promise((r) => setTimeout(r, 100));
  ...
  it('enable-disable-enable cycle writes correct sequences each time', async () => {
    ...
    instance = await renderInk(React.createElement(Text, null, 'cycle'), { stdout: mock.stdout });
  ...
  it('multiple unmounts are idempotent', async () => {
    ...
    instance = await renderInk(React.createElement(Text, null, 'double-unmount'), {
      stdout: mock.stdout,
    });
    await new Promise((r) => setTimeout(r, 100));
