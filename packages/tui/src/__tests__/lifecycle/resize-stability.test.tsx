...
describe('resize layout stability', () => {
  ...
  it('narrowing columns produces output within new width', async () => {
    ...
    for (const line of lines) {
      ...
      expect(width, `Line exceeds 60 cols (${width}): "${line.slice(0, 60)}"`).toBeLessThanOrEqual(60)
    }
  })
  ...
  it('widening columns produces output within new width', async () => {
    ...
    for (const line of lines) {
      if (line.trim().length === 0) continue
      const width = stringWidth(line)
      expect(width, `Line exceeds 120 cols (${width}): "${line.slice(0, 80)}"`).toBeLessThanOrEqual(120)
    }
  })
  ...
  it('rapid consecutive resizes converge to final dimensions', async () => {
    ...
    for (const line of lines) {
      if (line.trim().length === 0) continue
      const width = stringWidth(line)
      expect(width, `Line exceeds 80 cols after rapid resize (${width}): "${line.slice(0, 80)}"`).toBeLessThanOrEqual(80)
    }
