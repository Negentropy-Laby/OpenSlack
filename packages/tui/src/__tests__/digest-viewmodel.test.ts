  ...
  return {
    ...
    groups: [
      ...
      },
      {
        label: 'Needs Human',
        events: [
          makeEvent({ type: 'pr.merge.requested', summary: 'PR needs merge', nextAction: { owner: 'human', action: 'Approve merge' } }),
        ],
      },
      {
        label: 'Blocked',
        events: [
          makeEvent({ type: 'task.blocked', summary: 'Task blocked' }),
        ],
      },
    ],
    recommendedNext: [
      makeEvent({ type: 'pr.merge.requested', nextAction: { owner: 'human', action: 'Approve PR #42' } }),
    ],
    ...overrides,
  }
}
