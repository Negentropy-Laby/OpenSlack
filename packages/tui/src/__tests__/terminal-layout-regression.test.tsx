...
import { CharPool, HyperlinkPool, StylePool, createScreen, setCellAt } from '../ink/screen.js';
...
function makeFrame(lines: string[], width = 40, rows = 12): { frame: Frame; stylePool: StylePool } {
  ...
  const screen = createScreen(width, lines.length, stylePool, new CharPool(), new HyperlinkPool());
...
describe('terminal layout regressions', () => {
  ...
  it('clears stale same-line suffix when append-only output shrinks within a row', () => {
    const { frame: longFrame, stylePool } = makeFrame([
      'Review pull requests',
      'Check open PRs and merge readiness',
    ]);
  ...
  it('redraws workflow lifecycle after async loader data arrives', async () => {
    let resolveLoader:
      | ((value: ReturnType<typeof mapWorkflowLifecycleToViewModel>) => void)
      | undefined;
