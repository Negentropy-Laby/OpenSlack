...
describe('mapWorkflowPreviewToViewModel integration scenarios', () => {
  it('maps a realistic multi-phase workflow preview', () => {
    const preview: WorkflowPreview = {
      ...
      steps: [
        { phase: 'Research', type: 'action', title: 'Analyze codebase', actionId: 'analyze', sideEffects: false, requiresConfirmation: false },
        { phase: 'Research', type: 'action', title: 'Identify affected modules', actionId: 'identify', sideEffects: false, requiresConfirmation: false },
        { phase: 'Plan', type: 'action', title: 'Create implementation plan', actionId: 'plan', sideEffects: false, requiresConfirmation: false },
        { phase: 'Plan', type: 'decision-gate', title: 'Approve plan', sideEffects: false, requiresConfirmation: true, requiredRole: 'human' },
        { phase: 'Execute', type: 'action', title: 'Implement changes', actionId: 'implement', sideEffects: true, requiresConfirmation: false },
        { phase: 'Execute', type: 'action', title: 'Write tests', actionId: 'test', sideEffects: true, requiresConfirmation: false },
        { phase: 'Review', type: 'handoff', title: 'Handoff to reviewer', sideEffects: true, requiresConfirmation: false },
        { phase: 'Review', type: 'wait', title: 'Wait for review approval', sideEffects: false, requiresConfirmation: false },
      ],
      errors: [],
    }
  ...
  it('maps a preview with validation errors', () => {
    const preview: WorkflowPreview = {
      ...
      steps: [],
      errors: ['Missing required input: repo', 'Unknown action ID: invalid-action', 'Phase has no steps'],
    }
  ...
  it('handles a single-phase read-only workflow', () => {
    const preview: WorkflowPreview = {
      ...
      steps: [
        { phase: 'Audit', type: 'action', title: 'Scan dependencies', actionId: 'scan-deps', sideEffects: false, requiresConfirmation: false },
        { phase: 'Audit', type: 'action', title: 'Check licenses', actionId: 'check-licenses', sideEffects: false, requiresConfirmation: false },
        { phase: 'Audit', type: 'record-decision', title: 'Record audit findings', sideEffects: false, requiresConfirmation: false },
      ],
      errors: [],
    }
