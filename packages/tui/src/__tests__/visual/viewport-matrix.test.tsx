...
async function renderAt(element: React.ReactElement, cols: number, rows: number): Promise<string> {
...
const VIEWS: ViewSpec[] = [
  ...
  },
  {
    ...
    create: () =>
      React.createElement(WorkflowLifecycleView, { model: createWorkflowLifecycleViewModel() }),
  },
  {
    ...
    create: () =>
      React.createElement(WorkflowWorkbenchView, {
        galleryModel: createWorkflowWorkbenchViewModel(),
      }),
  },
  {
    name: 'DashboardView',
  ...
  },
  {
    ...
    markers: ['Approvals'],
    create: () =>
      React.createElement(ApprovalCenterView, { model: createApprovalCenterViewModel() }),
  },
  {
    name: 'RoomView',
    needsNav: false,
    ...
    for (const view of VIEWS) {
      it(`${view.name} renders within ${viewport.cols} columns and contains markers`, async () => {
        ...
        for (const marker of view.markers) {
          expect(output, `${view.name} at ${viewport.label} missing marker: ${marker}`).toContain(
            marker,
          );
        }
