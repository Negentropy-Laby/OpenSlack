    ...
    items: [
      {
        ...
        workflowGate: { touched: true, criteria: [{ name: 'Build', passed: true }], overall: 'PASS' },
      },
      {
        prNumber: 102,
...
function createProfileModel(overrides?: Partial<ProfileViewModel>): ProfileViewModel {
  return {
    ...
    posts: [
      { title: 'First Post', date: '2026-05-30', summary: 'Summary of first post', sourcePath: 'posts/first.md', url: 'https://example.com/first' },
    ],
...
describe('concurrent data updates', () => {
  it('DashboardView: summary counts and activity data change simultaneously', async () => {
    ...
    const updated = createDashboardModel({
      ...
      handoffs: [
        { id: 'h1', from: 'agent-a', to: 'agent-b', status: 'open', context: 'Review', age: '1h' },
        { id: 'h2', from: 'agent-c', to: 'human-d', status: 'open', context: 'New task', age: '30m' },
        { id: 'h3', from: 'agent-e', to: 'agent-f', status: 'open', context: 'Bug fix', age: '10m' },
      ],
      decisions: [
      ...
      recentActivity: [
        { time: '12:00', type: 'decision.accepted', summary: 'Decision on architecture', actor: 'team-lead' },
        { time: '11:55', type: 'handoff.opened', summary: 'Handoff from agent-c to human-d', actor: 'agent-c' },
  ...
  it('RoomView: blockers, handoffs, decisions, and activity all change at once', async () => {
    ...
    const updated = createRoomModel({
      ...
      recentActivity: [
        ...
        { time: '11:45', type: 'pr.blocked', summary: 'PR blocked by failing CI', actor: 'openslack-bot' },
      ],
    })
  ...
  it('ApprovalCenterView: groups appear and disappear simultaneously', async () => {
    ...
    const multiGroup = createApprovalCenterModel({
      ...
      summary: { plans: 1, mergeRequests: 1, workflowEffects: 1, profileSyncs: 1, githubReviews: 1 },
    })
...
describe('progressive data growth', () => {
  it('DashboardView: data grows across 4 rerenders without throwing', async () => {
    ...
    const step2 = createDashboardModel({
      summary: { blockers: 1, handoffs: 1, decisions: 0 },
      blockers: [{ object: 'pr:42', summary: 'Missing approval', severity: 'high' }],
      handoffs: [{ id: 'h1', from: 'agent-a', to: 'agent-b', status: 'open', context: 'Review', age: '1h' }],
      decisions: [],
      recentActivity: [],
    })
