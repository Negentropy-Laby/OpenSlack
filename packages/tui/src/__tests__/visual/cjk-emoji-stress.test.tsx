  ...
  return {
    pendingApprovals: [
      makeItem(
        'plan-001',
        'plan',
        `${CJK_ZH} 部署计划 ${EMOJI_TITLE}`,
        `${CJK_JA} production deploy`,
      ),
      makeItem('merge-001', 'merge-request', `合并 PR #42 ${CJK_KO}`, `${MIXED} merge detail`),
      makeItem(
        'wf-001',
        'workflow-effect',
        `${CJK_JA} workflow effect 🔧`,
        `${CJK_ANSI} side effect`,
      ),
      makeItem('ps-001', 'profile-sync', `Profile ${CJK_KO} sync 🔄`, `${ANSI_BOLD} sync action`),
      makeItem(
        'gh-001',
        'github-review',
        `GitHub ${CJK_ZH} review ${EMOJI_TITLE}`,
        `${OSC_HYPERLINK} CODEOWNER`,
      ),
    ],
    groups: [
      {
      ...
      },
      {
        ...
        items: [
          makeItem(
            ...
            'profile-sync',
            `Profile ${CJK_KO} sync 🔄`,
            `${ANSI_BOLD} sync action`,
          ),
        ],
      },
      {
        category: 'github-review',
        label: `👀 ${CJK_KO} Reviews`,
        items: [
          makeItem(
            'gh-001',
            'github-review',
...
function createCjkRoomViewModel(): RoomViewModel {
  return {
    ...
    blockers: [
      {
        type: `${CJK_ZH}-check`,
        summary: `${ANSI_COLORED} CI ${CJK_JA} failing`,
        timestamp: '2026-06-01T08:00:00Z',
      },
      {
        type: `${CJK_KO}-approval`,
        summary: `${CJK_ANSI} missing approval ${LONG_URL}`,
    ...
    handoffs: [
      {
        ...
        to: `${CJK_JA}-agent`,
        status: 'open',
        context: `${MIXED} continue review`,
      },
    ],
    decisions: [
      {
        id: `${CJK_KO}-d-001`,
        topic: `${EMOJI_TITLE} approve layout`,
        decision: `${CJK_ZH}-pending`,
        status: 'active',
      },
    ],
    recentActivity: [
      {
        time: '12:00 PM',
        type: 'review.comment',
        summary: `${ANSI_COLORED} ${CJK_JA} commented`,
        actor: `${CJK_KO}-agent`,
      },
      {
        time: '11:30 AM',
        type: 'check.failed',
        summary: `${CJK_ANSI} CI ${LONG_URL}`,
        actor: `${EMOJI_TITLE}-bot`,
      },
      {
        time: '11:00 AM',
        type: `${OSC_HYPERLINK}`,
        summary: `${MIXED} mixed event`,
        actor: `${CJK_ZH}-ci`,
      },
    ],
  };
...
function createCjkProfileViewModel(): ProfileViewModel {
  return {
    ...
    checkGroups: [
      {
        key: 'source',
        label: `${CJK_ZH} Source ${CJK_JA}`,
        status: 'pass',
        detail: `${CJK_KO} commit ok ${LONG_URL}`,
      },
      {
        key: 'posts',
        label: `Posts ${EMOJI_TITLE}`,
        status: 'pass',
        detail: `${ANSI_COLORED} 1/1 ${CJK_ZH}`,
      },
      {
        key: 'target-marker',
        label: `Target ${CJK_KO} marker`,
        status: 'pass',
        detail: `${CJK_ANSI} marker present`,
      },
...
function createCjkWorkflowLifecycleViewModel(): WorkflowLifecycleViewModel {
  return {
    ...
    stages: [
      {
        name: 'proposal',
        label: `${CJK_ZH} Proposal`,
        status: 'complete',
        icon: 'check',
        issueNumber: 120,
        detail: `${ANSI_BOLD} ${CJK_JA} accepted`,
      },
      {
        name: 'review',
        label: `${CJK_KO} Review 🔍`,
    ...
    phaseIssues: [{ phase: `${ANSI_BOLD}proposal`, issueNumber: 120, status: `${CJK_JA}-closed` }],
    currentRun: {
...
function createCjkDashboardViewModel(): DashboardViewModel {
  return {
    ...
    blockers: [
      {
        object: `${ANSI_BOLD} PR #130 ${CJK_JA}`,
        summary: `${CJK_KO}: ${EMOJI_TITLE}`,
        owner: `${ANSI_COLORED}-lead`,
        nextAction: LONG_URL,
        severity: `${CJK_ANSI}-high`,
      },
    ],
    handoffs: [
      {
    ...
    recentActivity: [
      {
        time: '12:00 PM',
        type: 'pr.merged',
        summary: `${ANSI_BOLD} ${CJK_ZH} PR #127 merged ${EMOJI_TITLE}`,
        actor: `${CJK_JA}-agent`,
      },
...
describe.each(WIDTHS)('CJK/emoji stress at %d columns', (cols) => {
  ...
  describe('RoomView', () => {
    it('renders CJK blockers and handoffs within width', async () => {
      ...
      const output = await renderAt(React.createElement(RoomView, { model }), cols);
      assertNoLineExceedsWidth(output, cols);
      assertNoAnsiInOutput(output);
