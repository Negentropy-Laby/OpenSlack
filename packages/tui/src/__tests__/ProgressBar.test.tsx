...
describe('ProgressBar render', () => {
  ...
  async function renderBar(jsx: React.ReactElement): Promise<string> {
    ...
    instance = await render(React.createElement(ThemeProvider, { mode: 'dark' }, jsx), {
      stdout,
      patchConsole: false,
    });
  ...
  it('clamps value exceeding max to 100%', async () => {
    const output = await renderBar(
      React.createElement(ProgressBar, { value: 150, max: 100, width: 10 }),
    );
    expect(output).toContain('100%');
  });
  ...
  it('handles max=0 without NaN', async () => {
    const output = await renderBar(
      React.createElement(ProgressBar, { value: 0, max: 0, width: 10 }),
    );
    expect(output).toContain('0%');
