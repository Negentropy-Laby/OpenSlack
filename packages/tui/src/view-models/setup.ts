...
function mapStatus(status: SetupFinding['status']): SetupFindingStatus {
  switch (status) {
    case 'ok':
      return 'PASS';
    case 'fixable_by_command':
      return 'WARN';
    case 'requires_github_admin':
      return 'FAIL';
...
export function mapSetupToViewModel(report: SetupReport): SetupViewModel {
  const fixable = report.findings.filter((f) => f.status === 'fixable_by_command').map(mapFinding);
  ...
  const readiness: SetupReadiness = allOk
    ? 'ready'
