...
function resolveAuthorDisplay(authorId: string, participants: AgentParticipant[]): string {
  ...
  switch (msg.kind) {
    ...
    case 'tool_event':
      return {
        ...
        metadata: {
          ...(msg.input != null
            ? { input: typeof msg.input === 'string' ? s(msg.input) : JSON.stringify(msg.input) }
            : {}),
          ...(msg.output != null
            ? {
                output: typeof msg.output === 'string' ? s(msg.output) : JSON.stringify(msg.output),
              }
            : {}),
        },
      };
