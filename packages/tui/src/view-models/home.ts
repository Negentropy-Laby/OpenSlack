  ...
  if (data?.shellData) {
    ...
    if (sd.prQueue && sd.prQueue.totalPRs > 0) {
      ...
      attentionItems.push({
        ...
        detail: blockedPr ? `#${blockedPr.prNumber} ${s(blockedPr.title)}` : detailParts.join(', '),
        route: 'pr-queue',
    ...
    if (sd.dashboard && sd.dashboard.summary.handoffs > 0) {
      ...
      attentionItems.push({
        ...
        detail: firstHandoff ? `${s(firstHandoff.from)} -> ${s(firstHandoff.to)}` : '',
        route: 'handoffs',
        colorTheme: 'warning',
      });
    }
...
function deriveRecommendedAction(
  ...
  if (approvalTotal > 0 && sd.approvals) {
    ...
    candidates.push({
      ...
      reason: s(
        `${approvalTotal} item${approvalTotal !== 1 ? 's' : ''} awaiting approval, risk: ${highestRisk.risk}`,
      ),
      route: 'approvals',
      urgency: 'governance',
  ...
  if (sd.workflowGallery) {
    ...
    if (awaiting.length > 0) {
      candidates.push({
        ...
        reason: s(
          `${awaiting.length} workflow${awaiting.length !== 1 ? 's' : ''} awaiting confirmation`,
        ),
        route: 'workflows',
        urgency: 'governance',
        priority: 5,
  ...
  if (sd.prQueue && sd.prQueue.blockedCount > 0) {
    candidates.push({
      label: s(
        `Review ${sd.prQueue.blockedCount} blocked PR${sd.prQueue.blockedCount !== 1 ? 's' : ''}`,
      ),
      reason: s(
        `${sd.prQueue.blockedCount} PR${sd.prQueue.blockedCount !== 1 ? 's are' : ' is'} blocked and cannot merge`,
      ),
      route: 'pr-queue',
      urgency: 'blocker',
  ...
  if (sd.status && sd.status.gitHub.tasksBlocked > 0) {
    candidates.push({
      label: s(
        `Resolve ${sd.status.gitHub.tasksBlocked} blocked task${sd.status.gitHub.tasksBlocked !== 1 ? 's' : ''}`,
      ),
      reason: s(
        `${sd.status.gitHub.tasksBlocked} task${sd.status.gitHub.tasksBlocked !== 1 ? 's' : ''} unable to proceed`,
      ),
      route: 'status',
      urgency: 'blocker',
  ...
  if (sd.dashboard && sd.dashboard.summary.handoffs > 0) {
    ...
    candidates.push({
      ...
      reason: s(
        `${sd.dashboard.summary.handoffs} open handoff${sd.dashboard.summary.handoffs !== 1 ? 's' : ''}`,
      ),
      route: 'handoffs',
      urgency: 'operational',
  ...
  if (sd.decisions && sd.decisions.activeCount > 0) {
    ...
    candidates.push({
      ...
      reason: s(
        `${sd.decisions.activeCount} active decision${sd.decisions.activeCount !== 1 ? 's' : ''} pending input`,
      ),
      route: 'decisions',
      urgency: 'operational',
  ...
  if (sd.prQueue && sd.prQueue.readyCount > 0) {
    candidates.push({
      ...
      reason: s(
        `${sd.prQueue.readyCount} PR${sd.prQueue.readyCount !== 1 ? 's' : ''} ready to merge`,
      ),
      route: 'pr-queue',
