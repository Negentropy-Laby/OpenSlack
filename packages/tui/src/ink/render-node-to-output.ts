...
import { isXtermJs } from './stubs/terminal.js';
...
// ── Gated frame diagnostics (P4-1f) ──
const ABY_PERF_DIAGNOSTICS =
  process.env.ABY_PERF_DIAGNOSTICS === '1' || process.env.ABY_PERF_DIAGNOSTICS === 'true';

let frameId = 0;
let frameNodeCount = 0;
let frameCacheHit = 0;
let frameCacheMiss = 0;
let frameCulledCount = 0;
let frameWrappedChars = 0;
let frameWrapCacheHit = 0;
let frameWrapCacheMiss = 0;
let frameTerminalLayoutHit = 0;
let frameTerminalLayoutMiss = 0;
let frameSegmentCount = 0;
let frameWrappedLineCount = 0;
...
function resetFrameMetrics() {
  ...
  frameWrappedChars = 0;
  frameWrapCacheHit = 0;
  frameWrapCacheMiss = 0;
  frameTerminalLayoutHit = 0;
  frameTerminalLayoutMiss = 0;
  frameSegmentCount = 0;
  frameWrappedLineCount = 0;
  frameStartTime = performance.now();
  frameActive = true;
}

function emitFrameMetrics() {
  ...
  console.warn(
    '[aby:perf] ' +
      JSON.stringify({
        event: 'ink_frame_diagnostics',
        frameId,
        nodeCount: frameNodeCount,
        cacheHit: frameCacheHit,
        cacheMiss: frameCacheMiss,
        culledCount: frameCulledCount,
        wrappedChars: frameWrappedChars,
        wrapCacheHit: frameWrapCacheHit,
        wrapCacheMiss: frameWrapCacheMiss,
        terminalLayoutHit: frameTerminalLayoutHit,
        terminalLayoutMiss: frameTerminalLayoutMiss,
        segmentCount: frameSegmentCount,
        wrappedLineCount: frameWrappedLineCount,
...
export function isDecstbmFastPathSafe(
  ...
  return (
    !hint ||
    (!hasRemovedChild && (heightDelta === 0 || (hint.delta > 0 && heightDelta === hint.delta)))
  );
}

function consumePendingClears(node: DOMElement, output: Output): boolean {
...
// ── Per-frame wrap cache (3g) ──
// Wrap results are pure functions of (text, maxWidth, textWrap).
// Caching avoids re-wrapping stable text nodes across frames.
const WRAP_CACHE_MAX = 256;
const wrapResultCache = new Map<string, { wrapped: string; softWrap: boolean[] | undefined }>();

function getWrapCacheKey(text: string, maxWidth: number, textWrap: string): string {
  return `${textWrap}:${maxWidth}:${text}`;
}
...
function wrapWithSoftWrap(
  ...
  const cacheKey = getWrapCacheKey(plainText, maxWidth, textWrap);
  const cached = wrapResultCache.get(cacheKey);
  if (cached) {
    if (ABY_PERF_DIAGNOSTICS && frameActive) {
      frameWrapCacheHit++;
      if (
        (textWrap === 'wrap' || textWrap === 'wrap-trim') &&
        !hasTerminalControlSequence(plainText)
      ) {
        frameTerminalLayoutHit++;
      }
    }
    return cached;
  }

  let result: { wrapped: string; softWrap: boolean[] | undefined };
  if ((textWrap === 'wrap' || textWrap === 'wrap-trim') && !hasTerminalControlSequence(plainText)) {
    // Direct terminal layout — returns softWrap aligned with output lines
    const layout = wrapTerminalText(plainText, maxWidth, {
      trim: textWrap === 'wrap-trim',
    });
    result = { wrapped: layout.wrapped, softWrap: layout.softWrap };
    if (ABY_PERF_DIAGNOSTICS && frameActive) {
      frameTerminalLayoutMiss++;
      frameSegmentCount += layout.segmentCount;
      frameWrappedLineCount += layout.wrapped.split('\n').length;
    }
  } else if (textWrap === 'wrap' || textWrap === 'wrap-trim') {
    // ANSI fallback — compute softWrap per original-line wrap
  ...
  wrapResultCache.set(cacheKey, result);
  if (wrapResultCache.size > WRAP_CACHE_MAX) {
    const first = wrapResultCache.keys().next().value;
    wrapResultCache.delete(first);
  }
...
function renderNodeToOutput(
  ...
  const track = ABY_PERF_DIAGNOSTICS;
  const isRoot = track && node.nodeName === 'ink-root';
  if (isRoot) {
    resetFrameMetrics();
  }
  if (track && frameActive) {
    frameNodeCount++;
  }

  _renderNodeToOutput(node, output, options);
...
function _renderNodeToOutput(
  ...
  if (yogaNode) {
    ...
    const hasRemovedChild = consumePendingClears(node, output);
    if (hasRemovedChild) {
      layoutShifted = true;
    }
    ...
    if (node.nodeName === 'ink-raw-ansi') {
      ...
      if (plainText.length > 0) {
        ...
        if (needsWrapping && ABY_PERF_DIAGNOSTICS && frameActive) {
          frameWrappedChars += plainText.length;
        }
      ...
      if (isScrollY) {
        ...
        const contentHasRemovedChild =
          content !== undefined ? consumePendingClears(content, output) : false;
        const scrollHasRemovedChild = hasRemovedChild || contentHasRemovedChild;
        if (contentHasRemovedChild) {
          layoutShifted = true;
        }
        ...
        if (content && contentYoga) {
          ...
          const safeForFastPath = isDecstbmFastPathSafe(hint, heightDelta, scrollHasRemovedChild);
