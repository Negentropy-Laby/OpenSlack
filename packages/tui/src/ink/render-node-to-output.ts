...
const ABY_PERF_DIAGNOSTICS =
  process.env.ABY_PERF_DIAGNOSTICS === '1' ||
  process.env.ABY_PERF_DIAGNOSTICS === 'true'
...
export function isDecstbmFastPathSafe(
  ...
  return (
    !hint ||
    (!hasRemovedChild &&
      (heightDelta === 0 || (hint.delta > 0 && heightDelta === hint.delta)))
  )
}
...
const wrapResultCache = new Map<
  string,
  { wrapped: string; softWrap: boolean[] | undefined }
>()

function getWrapCacheKey(
  text: string,
  maxWidth: number,
  textWrap: string,
): string {
    ...
    if (node.nodeName === 'ink-raw-ansi') {
      ...
      if (isScrollY) {
        ...
        if (content && contentYoga) {
          ...
          const safeForFastPath = isDecstbmFastPathSafe(
            hint,
            heightDelta,
