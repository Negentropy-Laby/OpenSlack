...
import { flushInteractionTime } from './stubs/bootstrap-state.js';
...
export default class Ink {
  ...
  private lastLayoutWidth: number = -1;
  ...
  constructor(private readonly options: Options) {
    ...
    this.log = new LogUpdate({
      ...
      appendOnlyOffscreenReset: hasCursorUpViewportYankBug()
    });
    ...
    this.rootNode.onComputeLayout = () => {
      ...
      if (this.rootNode.yogaNode) {
        const widthChanged = this.terminalColumns !== this.lastLayoutWidth;
        if (widthChanged) {
        ...
        if (widthChanged || this.rootNode.yogaNode.isDirty()) {
          ...
          this.lastLayoutWidth = this.terminalColumns;
  ...
  dispatchKeyboardEvent(parsedKey: ParsedKey): void {
    ...
    if (!(event as any).defaultPrevented && parsedKey.name === 'tab' && !parsedKey.ctrl && !parsedKey.meta) {
      if (parsedKey.shift) {
  ...
  // Stable identity for TerminalWriteContext. The constructor's autoBind(this)
  // binds this method once per Ink instance, avoiding per-render function churn
  // that would make <AlternateScreen> exit and re-enter on every render.
   ...
   * marks the next render as full-damage recovery. Not patching
