...
export function prepareTerminalText(
  ...
  function classifyAndAppend(char: string, cp: number): void {
    ...
    const forceSingleSegment =
      kind === 'text' && (KINSOKU_START.has(cp) || KINSOKU_END.has(cp))
...
export function layoutTerminalText(
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (lineWidth + segWidth <= columns) {
      ...
      const adjusted = adjustBreakForKinsoku(
        lineItems,
        breakItemCount,
        columns,
      )
      ...
      if (breakItemCount === snapItemCount && segments[breakAtIdx]!.kind === 'space') {
        while (
          resumeIdx < segments.length &&
          segments[resumeIdx]!.kind === 'space'
        ) {
          resumeIdx++
        }
      }
...
function adjustBreakForKinsoku(
  ...
  while (
    nextBreakItemCount > 1 &&
    endsWithKinsokuEnd(items[nextBreakItemCount - 1]!.text)
  ) {
    nextBreakItemCount--
    changed = true
  }
