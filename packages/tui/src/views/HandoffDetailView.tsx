...
export default function HandoffDetailView({
  model,
  onBack,
}: HandoffDetailViewProps): React.JSX.Element {
  const { exit } = useApp();
  ...
  const statusIcon =
    model.status === 'open' ? 'warn' : model.status === 'accepted' ? 'pass' : 'info';
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      model.acceptedAt
        ? React.createElement(ThemedText, { colorTheme: 'muted' }, `Accepted: ${model.acceptedAt}`)
        : null,
      model.closedAt
        ? React.createElement(ThemedText, { colorTheme: 'muted' }, `Closed: ${model.closedAt}`)
        : null,
      model.issueRef
        ? React.createElement(ThemedText, { colorTheme: 'info' }, `Issue: ${model.issueRef}`)
        : null,
      model.prRef
        ? React.createElement(ThemedText, { colorTheme: 'info' }, `PR: ${model.prRef}`)
        : null,
    ),
