...
export default function WorkflowWorkbenchView({
  galleryModel,
  actionHandlers,
}: WorkflowWorkbenchProps): React.JSX.Element {
  ...
  const makePreviewAction = useCallback(
    (wf: WorkflowGalleryItem): TuiAction => ({
      id: `preview-${wf.name}`,
  ...
  const makeDryRunAction = useCallback(
    (wf: WorkflowGalleryItem): TuiAction => ({
      id: `dry-run-${wf.name}`,
  ...
  const makeRunAction = useCallback(
    (wf: WorkflowGalleryItem): TuiAction => ({
      id: `run-${wf.name}`,
      ...
      handler: async (): Promise<TuiActionResult> => {
        ...
        return {
          ...
          data: {
            cliCommand: `openslack collaboration workflow run ${wf.name}`,
            workflow: wf.name,
          },
        };
      },
  ...
  const makeTrustAction = useCallback(
    (wf: WorkflowGalleryItem): TuiAction => {
      ...
      const nextLevel =
        currentIdx >= 0 && currentIdx < TRUST_LEVELS.length - 1
      ...
      return {
        ...
        handler: async (): Promise<TuiActionResult> => {
          ...
          return {
            ...
            data: {
              cliCommand: `openslack collaboration workflow trust ${wf.name} --level ${nextLevel}`,
              workflow: wf.name,
              from: currentTrust,
              to: nextLevel,
            },
  ...
  const makeStartPromptAction = useCallback(
    (prompt: string): TuiAction => ({
  ...
  const makeStartPatternAction = useCallback(
    (pattern: WorkflowStartPatternItem): TuiAction => ({
  ...
  const goToActionResult = useCallback(
    (returnMode: ViewMode = currentWf ? 'detail' : 'gallery') => {
  ...
  useInput((input, key) => {
    ...
    if (mode === 'gallery') {
      if (key.upArrow) {
        ...
        const publishAction: TuiAction =
          currentWf && actionHandlers?.publishWorkflowAsIssue
            ? {
            ...
            : {
                ...
                description:
                  'A workflow selection and publish handler are required before publishing.',
                requiresConfirmation: false,
                handler: async () => ({
                  success: false,
                  message: currentWf
    ...
    if (mode === 'action-result') {
      if (key.return) {
        const terminal =
          actionDispatch.state.status === TuiActionStatus.Success ||
          actionDispatch.state.status === TuiActionStatus.Error;
        if (terminal) {
          returnFromActionResult();
        }
      }
  ...
  const renderTrustInfo = (wf: WorkflowGalleryItem) => {
    ...
    return React.createElement(
      ...
      React.createElement(StatusIcon, {
        status: trustCat === 'pass' ? 'pass' : trustCat === 'warn' ? 'warn' : 'blocked',
      }),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              '  generate a safe draft from a task prompt',
            ),
          ),
          React.createElement(
            Box,
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'muted', dim: true },
              '  choose a Dynamic Workflow pattern',
            ),
          ),
          React.createElement(
            Box,
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'muted', dim: true },
              '  preview, dry-run, or run a discovered workflow',
            ),
          ),
        ),
      ),
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ThemedText,
            { colorTheme: promptInput.length > 0 ? 'foreground' : 'muted' },
            displayPrompt,
          ),
          React.createElement(
            ThemedText,
            { colorTheme: isAtPromptLimit ? 'warning' : 'muted', dim: !isAtPromptLimit },
            `Prompt length: ${promptInput.length}/${MAX_PROMPT_INPUT_LENGTH}`,
          ),
          React.createElement(
            ThemedText,
            { colorTheme: 'muted', dim: true },
            'Creates a draft in .openslack/workflows/drafts/ using workflow start semantics.',
          ),
        ),
      ),
      React.createElement(Divider, { length: 40 }),
    ...
    const patternRows = patterns.map((pattern, i) => {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ThemedText,
            { colorTheme: isSelected ? 'accent' : 'muted' },
            isSelected ? '>' : ' ',
          ),
          React.createElement(Text, null, ' '),
          React.createElement(
      ...
      patterns.length > 0
        ...
        : React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'muted' },
              'No Dynamic Workflow patterns discovered.',
            ),
          ),
      React.createElement(Divider, { length: 40 }),
        ...
        actionDispatch.state.result?.data?.cliCommand
          ? React.createElement(
              ...
              React.createElement(
                ThemedText,
                { colorTheme: 'accent' },
                String(actionDispatch.state.result.data.cliCommand),
              ),
            )
          : null,
      ),
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: formatColorTheme(wf.format) },
              wf.format.toUpperCase(),
            ),
          ),
          renderTrustInfo(wf),
          React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ThemedText,
            { colorTheme: actionHandlers?.publishWorkflowAsIssue ? 'foreground' : 'muted' },
            '[u] Publish workflow to GitHub Issues',
          ),
          React.createElement(
            ThemedText,
            { colorTheme: actionHandlers?.requestWorkflowReview ? 'foreground' : 'muted' },
            '[v] Request workflow review',
          ),
          React.createElement(
            ThemedText,
            { colorTheme: actionHandlers?.splitWorkflowIntoIssues ? 'foreground' : 'muted' },
            '[m] Split workflow phases into issues',
          ),
        ),
      ),
      React.createElement(Divider, { length: 40 }),
  ...
  const galleryRows = items.map((wf, i) => {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ThemedText,
          { colorTheme: formatColorTheme(wf.format), dim: true },
          wf.format.toUpperCase(),
        ),
      ),
      React.createElement(
  ...
  const workflowHomeActions = [
    {
      key: '1',
      label: 'Start',
      detail: 'Generate from prompt, choose pattern, or open a saved workflow',
    },
  ...
  return React.createElement(
    ...
    React.createElement(
      Pane,
      { title: 'Dynamic Workflows', marginY: 0 },
      React.createElement(
        Box,
        { flexDirection: 'column' },
        ...workflowHomeActions.map((action) =>
          React.createElement(
            Box,
            { key: action.key, flexDirection: 'column' },
            React.createElement(
              Box,
              { flexDirection: 'row' },
    ...
    items.length > 0
      ? React.createElement(
          Pane,
          { title: 'Workflow Gallery', marginY: 0 },
