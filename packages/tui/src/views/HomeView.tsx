...
export type HomeViewProps = {
  ...
  onAskSubmit?: (
    input: AskBarSubmit,
    threadId?: string,
  ) => Promise<TuiAskResult | void> | TuiAskResult | void;
...
export function resolveAskHistorySelection(
  ...
  if (direction === 'older') {
    const cursor =
      historyCursor === undefined ? 0 : Math.min(historyCursor + 1, askHistory.length - 1);
...
export async function executeConversationActionCard({
  ...
  try {
    ...
    if (card.kind === 'workflow_draft') {
      ...
      const message = card.command
        ? `Use: ${card.command}`
        : 'Workflow draft handler is not available.';
      setAskMessage(message);
      await record(message);
      return;
    }
    ...
    const message = card.command
      ? `Use: ${card.command}`
      : `${card.label} is not executable in this TUI session.`;
    setAskMessage(message);
    await record(message);
  } catch (err) {
...
export default function HomeView({
  ...
  const executeCard = useCallback(
    async (card: ConversationActionCard) => {
  ...
  for (let i = 0; i < taskCount; i++) {
    ...
    taskElements.push(
      renderTaskRow(item, isSelected, i, handleItemClick, handleItemHover, compactHome),
    );
  }
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(ThemedText, { colorTheme: 'accent', bold: true }, 'Suggested shortcuts'),
      React.createElement(Box, { flexDirection: 'column' }, ...taskElements),
    ),
    ...
    ...(compactHome
      ...
      : [
          ...
          React.createElement(
            ...
            React.createElement(Box, { flexDirection: 'column' }, ...navElements),
          ),
          ...
          React.createElement(
            ...
            React.createElement(KeyboardShortcutHint, {
              keys: ['Enter'],
              description: 'Ask/select',
            }),
            React.createElement(Text, null, '  '),
            React.createElement(KeyboardShortcutHint, {
              keys: ['Down'],
              description: 'suggestions/menu',
            }),
            React.createElement(Text, null, '  '),
...
function renderTaskRow(
  ...
  const detailElement =
    item.detail && (!compact || isSelected)
      ? React.createElement(
          Box,
          { marginLeft: 4 },
