...
export default function ApprovalCenterView({
  ...
  const isExternalGithubReview = selected
    ? !isTuiConfirmableApprovalCategory(selected.category)
    : false;

  /** Build a TuiAction for the selected item. */
  const buildAction = useCallback(
    (isApprove: boolean): TuiAction | null => {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(StatusIcon, {
              status: selected.risk === 'low' ? 'pass' : selected.risk === 'high' ? 'fail' : 'warn',
            }),
            ...
            ? React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement(
                    ThemedText,
                    { colorTheme: 'accent' },
                    getCliSuggestion(selected.category, true, selected.prNumber),
                  ),
                ),
                React.createElement(
                  Box,
                  ...
                  React.createElement(
                    ThemedText,
                    { colorTheme: 'foreground' },
                    'disabled for GitHub review approvals',
                  ),
                ),
              )
            : null,
            ...
            ? React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement(
                    ThemedText,
                    { colorTheme: 'error' },
                    'Self-merge, auto-merge, Black Zone merge',
                  ),
                ),
              )
            : null,
          ...
          selected.category === 'workflow-effect'
            ? React.createElement(
                ...
                React.createElement(
                  ThemedText,
                  { colorTheme: 'foreground' },
                  'TUI confirmation resumes or cancels the workflow. GitHub PR approval is separate.',
                ),
              )
            : null,
          ...
          selected.category === 'plan'
            ? React.createElement(
                ...
                React.createElement(
                  ThemedText,
                  { colorTheme: 'foreground' },
                  'TUI confirmation approves or rejects the plan. This is not a GitHub PR approval.',
                ),
              )
            : null,
          React.createElement(
  ...
  for (const group of model.groups) {
    ...
    for (const item of group.items) {
      ...
      const categoryIcon =
        item.category === 'plan'
          ? 'pass'
          : item.category === 'merge-request'
            ? 'warn'
            : item.category === 'workflow-effect'
              ? 'info'
              : item.category === 'profile-sync'
                ? 'info'
                : 'blocked';
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'muted', dim: true },
              `${getCategoryLabel(item.category)} — ${item.requestedBy}`,
            ),
          ),
        ),
      );
  ...
  return React.createElement(
    ...
    items.length > 0
      ? React.createElement(
          Pane,
          { title: 'Pending Approvals', marginY: 0 },
