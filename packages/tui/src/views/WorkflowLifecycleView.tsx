...
export default function WorkflowLifecycleView({
  ...
  actionHandlers,
  onBack,
}: WorkflowLifecycleViewProps): React.JSX.Element {
    ...
    return {
      ...
      handler: async (): Promise<TuiActionResult> => {
        ...
        return {
          ...
          data: {
            cliCommand: `openslack collaboration workflow run ${model.workflowName}`,
  ...
  React.useEffect(() => {
    if (
      mode === 'detail' &&
      (actionState.status === TuiActionStatus.Success ||
        actionState.status === TuiActionStatus.Error)
    ) {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          `OpenSlack / Lifecycle / ${model.workflowName}`,
        ),
      ),
          ...
          actionState.result?.data?.cliCommand
            ? React.createElement(
                ...
                React.createElement(
                  ThemedText,
                  { colorTheme: 'accent' },
                  String(actionState.result.data.cliCommand),
                ),
              )
            : null,
        ),
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ThemedText,
        { colorTheme: trustColorTheme(model.trustLevel) },
        model.trustLevel,
      ),
      React.createElement(Text, null, '  '),
      ...
      model.subIssueMode
        ? React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: subIssueModeColorTheme(model.subIssueMode) },
              model.subIssueMode,
            ),
          )
        : null,
      model.dependencyMode
        ? React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: dependencyModeColorTheme(model.dependencyMode) },
              model.dependencyMode,
            ),
          )
        : null,
    ...
    stages.length > 0
      ? React.createElement(
          ...
          React.createElement(Box, { flexDirection: 'row', flexWrap: 'wrap' }, ...progressBarNodes),
      ...
      : React.createElement(
          ...
          React.createElement(
            ThemedText,
            { colorTheme: 'muted' },
            'No GitHub issues found for this workflow.',
          ),
          React.createElement(
            Box,
            { flexDirection: 'row', marginTop: 1 },
      ...
      ? React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'foreground' },
              String(model.phaseIssues.length),
            ),
          ),
          model.phaseIssues.map((pi: PhaseIssueItem) =>
            React.createElement(
              ...
              React.createElement(
                ThemedText,
                { colorTheme: 'muted', dim: true },
                pi.issueNumber ? `#${pi.issueNumber} ${pi.phase}` : pi.phase,
              ),
            ),
          ),
        )
      ...
      ? React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ThemedText,
              { colorTheme: 'foreground' },
              String(model.currentRun.phaseIndex),
            ),
          ),
        )
      : null,
    ...
    model.prNumber
      ? React.createElement(
          ...
          React.createElement(
            ThemedText,
            { colorTheme: prStatusColorTheme(model.prStatus) },
            model.prStatus ?? '',
          ),
        )
      : null,
