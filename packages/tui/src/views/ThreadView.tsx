...
export default function ThreadView({ model, onBack }: ThreadViewProps): React.JSX.Element {
  ...
  const statusCat =
    model.status === 'active' ? 'pass' : model.status === 'paused' ? 'warn' : 'info';
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        `Status: ${model.status} · ${model.participants.length} participants`,
      ),
    ),
    ...
    model.messages.length > 0
      ? React.createElement(
          ...
          ...model.messages.map((msg) =>
            React.createElement(Box, { key: msg.id, flexDirection: 'column' }, renderMessage(msg)),
          ),
        )
