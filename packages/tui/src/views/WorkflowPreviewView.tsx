...
function stepStatus(model: WorkflowPreviewViewModel, step: WorkflowPreviewViewModel['steps'][number]): 'PASS' | 'FAIL' | 'WARN' | 'info' {
...
export default function WorkflowPreviewView({ model }: WorkflowPreviewViewProps): React.JSX.Element {
  const { exit } = useApp()
  ...
  return React.createElement(
    ...
    // Header
    React.createElement(ThemedText, { colorTheme: 'accent', bold: true }, `Workflow: ${model.name}`),
    React.createElement(ThemedText, { colorTheme: 'muted', dim: true }, `Template: ${model.templateId} | Correlation: ${model.correlationId}`),
    ...
    React.createElement(
      ...
      React.createElement(StatusIcon, { category: model.hasErrors ? 'fail' : model.hasSideEffects ? 'warn' : 'pass' }),
      ...
      React.createElement(Text, null, ' '),
      React.createElement(ThemedText, { colorTheme: 'foreground' }, model.hasSideEffects ? 'Has side effects' : 'Read-only'),
    ),
