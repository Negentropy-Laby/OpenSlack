 ...
 */
export function diagnoseTui(stdout?: {
  isTTY?: boolean;
  columns?: number;
  rows?: number;
}): TuiDiagnosticReport {
  ...
  findings.push({
    ...
    detail: isTTY ? 'Output is a TTY device' : 'Output is not a TTY (piped or redirected)',
    fallback: 'Run in an interactive terminal. For CI, use: openslack status --format standard',
  });

  // 2. Terminal dimensions
  const dimStatus: DiagnosticStatus =
    columns >= MIN_COLUMNS && rows >= MIN_ROWS ? 'pass' : columns > 0 && rows > 0 ? 'warn' : 'fail';
  findings.push({
    label: 'Dimensions',
    status: dimStatus,
    detail:
  ...
  findings.push({
    ...
    fallback:
      'CI terminals do not support interactive TUI. Use: openslack status --format standard',
  });
  ...
  const recommendedMode: 'tui' | 'plain' = summary.fail > 0 || (isCI && !isTTY) ? 'plain' : 'tui';
...
export function renderDiagnosticsPlain(report: TuiDiagnosticReport): string {
  ...
  for (const finding of report.findings) {
    const icon =
      finding.status === 'pass' ? '[PASS]' : finding.status === 'warn' ? '[WARN]' : '[FAIL]';
