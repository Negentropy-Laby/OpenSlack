...
function getBlockerLabel(decision: PRReviewState): string {
  switch (decision) {
    case 'BLOCKED_DRAFT': return 'PR is in draft state';
    case 'BLOCKED_POLICY': return 'Policy violation';
    case 'BLOCKED_BLACK_ZONE': return 'Black Zone — never mergeable';
    case 'BLOCKED_SELF_REVIEW': return 'Self-review detected';
    case 'BLOCKED_AUTHOR_IS_SOLE_CODEOWNER': return 'Author is sole CODEOWNER';
    case 'BLOCKED_SINGLE_MAINTAINER': return 'Single maintainer deadlock';
    case 'CHECKS_PENDING': return 'Checks still running';
    case 'CHECKS_FAILED': return 'Checks failed';
    case 'NEEDS_HUMAN_APPROVAL': return 'Missing valid human approval';
    case 'NEEDS_CODEOWNER_APPROVAL': return 'Missing CODEOWNER approval';
    case 'NEEDS_CHANGES': return 'Changes requested';
    case 'BOT_APPROVAL_IGNORED': return 'Bot approval ignored';
    default: return 'Cannot merge';
  }
}

function getNextAction(decision: PRReviewState): string {
  switch (decision) {
    case 'BLOCKED_DRAFT': return 'Mark the PR as ready for review.';
    case 'BLOCKED_POLICY': return 'Resolve the policy issue (state, conflicts).';
    ...
    case 'BOT_APPROVAL_IGNORED': return 'Request review from an independent human reviewer on GitHub.';
    case 'NEEDS_CODEOWNER_APPROVAL': return 'Request review from CODEOWNERS on GitHub.';
    case 'NEEDS_CHANGES': return 'Address the requested changes.';
    default: return 'Review the PR on GitHub, then ask OpenSlack to merge.';
  }
}
