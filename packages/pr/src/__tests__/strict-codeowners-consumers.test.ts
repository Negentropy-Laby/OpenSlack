...
describe('strict CODEOWNERS evidence consumers', () => {
  beforeEach(() => {
    ...
    hoisted.listOpenPRs.mockResolvedValue([{ number: 42, title: 'PR without CODEOWNERS evidence' }]);
  });

  it('makes watch fail closed instead of treating missing CODEOWNERS as no owners', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});

    await expect(watchPR(42, { timeoutSeconds: 1, intervalSeconds: 0 }))
