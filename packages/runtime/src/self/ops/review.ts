...
export function reviewPR(
  ...
  if (implementationAgent === reviewerAgent) {
    checks.push({
      ...
      passed: false,
      detail: 'Implementation agent cannot review own PR',
    });
  } else {
    checks.push({
      name: 'independent_review',
      passed: true,
      detail: `Reviewer ${reviewerAgent} != implementer ${implementationAgent}`,
    });
  }
  ...
  if (validation?.protectedPathCheck) {
    if (validation.protectedPathCheck.black_zone_touched) {
      checks.push({
        name: 'black_zone',
        passed: false,
        detail: 'Black Zone files touched — automatic rejection',
      });
    } else if (validation.protectedPathCheck.red_zone_touched) {
      checks.push({
        name: 'red_zone',
        passed: false,
        detail: 'Red Zone files touched — requires human approval',
      });
    } else {
  ...
  if (validation?.score) {
    if (validation.score.overall < 0.7) {
      checks.push({
        name: 'fitness',
        passed: false,
        detail: `Fitness score ${validation.score.overall.toFixed(3)} < 0.70 threshold`,
      });
    } else if (validation.score.overall < 0.85) {
      checks.push({
        name: 'fitness',
        passed: true,
        detail: `Fitness score ${validation.score.overall.toFixed(3)} — manual review recommended`,
      });
    } else {
      checks.push({
        name: 'fitness',
        passed: true,
        detail: `Fitness score ${validation.score.overall.toFixed(3)} >= 0.85`,
      });
    }
  }
