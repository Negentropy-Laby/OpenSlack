...
function extractPermissions(meta: WorkflowModuleShape['meta']): {
  read: string[];
  sideEffects: string[];
  forbidden: string[];
} {
...
function extractSourcePath(
  _meta: WorkflowModuleShape['meta'],
  module: WorkflowModuleShape,
): string {
...
function checkMinPermissions(meta: WorkflowModuleShape['meta']): boolean {
  ...
  const hasRead = Object.values(meta.permissions).some(
...
export async function publishWorkflowRunAudit(
  ...
  if (opts.createIssue === false) {
    throw new Error(
      'No issue number provided and createIssue is false. Cannot create audit record.',
    );
  }
...
export async function appendWorkflowRunPhaseComment(
  ...
  if (client.isDryRun) {
    console.log(
      `[DRY RUN] Would append phase comment to issue #${issueNumber}: ${phase} = ${status}`,
    );
    return { url: '' };
  }
...
export async function publishWorkflowSplit(
  ...
  opts: {
    ...
    linearDependencies?: boolean;
  },
): Promise<{
  parentIssueNumber: number;
  subIssues: PhaseSubIssue[];
  links: WorkflowSplitLinkSummary;
}> {
...
async function linkNativeSubIssues(
  ...
  for (const sub of subIssues) {
    ...
    try {
      ...
      console.log(
        `[WARNING] Failed to link sub-issue #${sub.issueNumber}: ${(err as Error).message}`,
      );
      fallbackReasons.push({
        kind: 'sub-issue',
        reason: 'native_sub_issue_error',
        issueNumber: sub.issueNumber,
      });
    }
  }
...
async function linkLinearDependencies(subIssues: PhaseSubIssue[]): Promise<{
  nativeLinked: number;
  fallbackLinked: number;
  fallbackReasons: WorkflowLinkFallback[];
}> {
  const client = await getClient();
  const fallbackReasons: WorkflowLinkFallback[] = [];
  if (client.isDryRun) {
    console.log(
      `[DRY RUN] Would create ${subIssues.length - 1} linear dependencies between phase issues`,
    );
  ...
  for (let i = 1; i < subIssues.length; i++) {
    ...
    if (!nativeDependencyUnavailable && blocker.issueId) {
      try {
        ...
        ).request('POST /repos/{owner}/{repo}/issues/{issue_number}/dependencies/blocked_by', {
          owner: client.owner,
          repo: client.repo,
...
Complete ${blocker.phase} before starting this phase.`,
      ...
      console.log(
        `[WARNING] Failed to link dependency ${blocker.issueNumber} → ${blocked.issueNumber}: ${(err as Error).message}`,
      );
    }
  }
...
async function addSubIssueFallbackComment(
  ...
  try {
    ...
    console.log(
      `[WARNING] Failed to add fallback comment to parent issue #${parentIssue}: ${(err as Error).message}`,
    );
  }
}
