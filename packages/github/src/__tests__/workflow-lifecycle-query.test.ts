...
describe('fetchWorkflowLifecycleIssues', () => {
  ...
  it('finds proposal, review, and split issues', async () => {
    const octokit = mockOctokit({
      proposal: [
        mockIssue({
          number: 10,
          title: '[Workflow Proposal] test-workflow',
      ...
      review: [
        mockIssue({
          number: 20,
          title: '[Workflow Review] test-workflow',
    ...
    expect(result.proposalIssue).toEqual({
      number: 10,
      state: 'open',
      url: 'https://github.com/test/repo/issues/10',
    });
    expect(result.reviewIssue).toEqual({
      number: 20,
      state: 'closed',
      url: 'https://github.com/test/repo/issues/20',
    });
    expect(result.splitIssue).toEqual({
      number: 30,
      state: 'open',
  ...
  it('extracts phase names from phase issue titles', async () => {
    const octokit = mockOctokit({
      ...
      phase: [
        ...
        mockIssue({
          number: 41,
          title: '[Workflow Phase] test-workflow / Fix',
          html_url: 'https://github.com/test/repo/issues/41',
          state: 'closed',
        }),
      ],
      run: [],
    ...
    expect(result.phaseIssues[0]).toEqual({
      number: 40,
      phase: 'Scan',
      state: 'open',
      url: 'https://github.com/test/repo/issues/40',
    });
    expect(result.phaseIssues[1]).toEqual({
      number: 41,
      phase: 'Fix',
      state: 'closed',
  ...
  it('extracts runId and status from run issue titles and bodies', async () => {
    ...
    expect(result.runIssues[0]).toEqual({
      number: 50,
      runId: 'run_001',
      state: 'open',
  ...
  it('finds improvement issues', async () => {
    const octokit = mockOctokit({
      ...
      improvement: [
        mockIssue({
          number: 60,
          title: '[Workflow Improvement] test-workflow',
    ...
    expect(result.improvementIssues[0]).toEqual({
      number: 60,
      state: 'open',
    ...
    const octokit = {
      issues: {
        listForRepo: vi.fn().mockImplementation(({ labels }: { labels: string }) => {
          ...
          const data =
            {
              split: [mockIssue({ number: 30, title: '[Workflow Split] test-workflow', body: '' })],
              phase: [
