...
vi.mock('../profile-sync.js', () => ({
  ...
  renderLatestInsightsSection: vi.fn(
    (posts: unknown[]) => `Rendered ${(posts as Array<unknown>).length} posts`,
  ),
...
describe('runProfileSync', () => {
  ...
  it('completes successfully on happy path', async () => {
    ...
    mockCreateProfileSyncPR.mockResolvedValue({
      url: 'https://github.com/owner/.github/pull/42',
      number: 42,
    });
    ...
    expect(recordEvent).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'profile_sync.completed' }),
    );
  });
  ...
  it('returns failed when marker is missing', async () => {
    ...
    mockPublishProfileSyncFailure.mockResolvedValue({
      issueNumber: 99,
      url: 'https://github.com/owner/.github/issues/99',
    });
    mockListOpenPRs.mockResolvedValue([]);
    ...
    expect(recordEvent).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'profile_sync.failed' }),
    );
  });
    ...
    mockListOpenPRs.mockResolvedValue([
      {
        ...
        url: 'https://github.com/owner/.github/pull/10',
        branch: 'openslack/profile-sync/latest-insights-20260530-deadbee-abc123',
      },
    ]);
  ...
  it('returns failed and creates failure issue on PR creation failure', async () => {
    ...
    mockPublishProfileSyncFailure.mockResolvedValue({
      issueNumber: 88,
      url: 'https://github.com/owner/.github/issues/88',
    });
    mockListOpenPRs.mockResolvedValue([]);
    ...
    expect(mockPublishProfileSyncFailure).toHaveBeenCalledWith(
      expect.objectContaining({ phase: 'pr' }),
    );
    expect(recordEvent).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'profile_sync.failed' }),
    );
