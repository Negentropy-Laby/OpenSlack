...
describe('workflow issue publishers', () => {
  ...
  describe('publishWorkflowProposal', () => {
    ...
    it('infers openslack-native format', async () => {
      ...
      await publishWorkflowProposal(
        mockModule({ format: 'openslack-native', sourceBody: undefined }),
        {
          requestedBy: 'test-user',
        },
      );
    ...
    it('appends comment to existing issue', async () => {
      const mockOctokit = {
        issues: {
          createComment: vi
            .fn()
            .mockResolvedValue({ data: { html_url: 'https://github.com/test/42#comment' } }),
        },
      };
      mockGetClient.mockResolvedValue({
        owner: 'test',
  ...
  describe('publishWorkflowSplit', () => {
    ...
    it('accepts nativeSubIssues and linearDependencies options', async () => {
      ...
      const mockOctokit = {
        ...
        issues: {
          createComment: vi
            .fn()
            .mockResolvedValue({ data: { html_url: 'https://github.com/test/comment' } }),
    ...
    it('falls back to structured dependency comments when native API is unavailable', async () => {
      ...
      const request = vi
        .fn()
        .mockRejectedValue(Object.assign(new Error('not enabled'), { status: 422 }));
      ...
      expect(result.links.fallbackReasons.map((r) => r.reason)).toContain(
        'native_sub_issues_unavailable_422',
      );
      expect(result.links.fallbackReasons.map((r) => r.reason)).toContain(
        'native_dependencies_unavailable_422',
      );
