...
function eventComment(type: string, issueNumber: number, details: Record<string, unknown>): string {
  const event = {
    schema: 'openslack.issue_event.v1',
...
export async function markIssueRunning(opts: MarkOptions): Promise<void> {
  ...
  if (client.isDryRun) {
    console.log(`[DRY RUN] markIssueRunning #${opts.issueNumber}`);
    return;
  }
  ...
  try {
    await client.octokit.issues.createComment({
      ...
      repo: client.repo,
      issue_number: opts.issueNumber,
      body: eventComment('task_running', opts.issueNumber, {
        run_id: opts.runId,
...
export async function markIssueBlocked(opts: MarkOptions): Promise<void> {
  ...
  if (client.isDryRun) {
    console.log(`[DRY RUN] markIssueBlocked #${opts.issueNumber}`);
    return;
  }
  for (const l of labelsToRemove) {
    try {
  ...
  try {
    await client.octokit.issues.createComment({
      ...
      repo: client.repo,
      issue_number: opts.issueNumber,
      body: eventComment('task_blocked', opts.issueNumber, {
        reason: opts.reason,
        requested_human_action: opts.requestedHumanAction,
      }),
    });
  } catch {
    /* ok */
...
export async function markIssueDone(opts: MarkOptions): Promise<void> {
  ...
  if (client.isDryRun) {
    console.log(`[DRY RUN] markIssueDone #${opts.issueNumber}`);
    return;
  }
  for (const l of labelsToRemove) {
    try {
