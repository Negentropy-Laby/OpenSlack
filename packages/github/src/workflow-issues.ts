...
export interface WorkflowMetaShape {
  ...
  inputs?: Record<
    string,
    { type: 'string' | 'number' | 'boolean'; default?: unknown; description: string }
  >;
...
export function renderWorkflowReviewBody(review: WorkflowReviewIssue): string {
  ...
  lines.push(
    `| Declared side effects | ${review.staticAnalysis.declaredSideEffects ? 'PASS' : 'FAIL'} |`,
  );
  lines.push('');
  lines.push('### Decision');
...
export function renderWorkflowRunPhaseComment(
  ...
  const emoji =
    status === 'completed' ? '✅' : status === 'failed' ? '❌' : status === 'paused' ? '⏸️' : '🔄';
  const lines: string[] = [];
...
export const WORKFLOW_LABEL_DEFINITIONS: Array<{
  name: string;
  color: string;
  description: string;
}> = [
  {
    ...
    color: 'b60205',
    description: 'Workflow artifact governance evidence',
  },
