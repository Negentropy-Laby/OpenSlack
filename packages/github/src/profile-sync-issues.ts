...
export function renderProfileSyncImprovementBody(improvement: ProfileSyncImprovementIssue): string {
  ...
  if (improvement.affectedPhase) lines.push(`affected_phase: ${JSON.stringify(improvement.affectedPhase)}`)
  lines.push('```')
  lines.push('')
  lines.push('---')
  lines.push('')
...
export const PROFILE_SYNC_LABEL_DEFINITIONS: Array<{ name: string; color: string; description: string }> = [
  { name: 'profile-sync:proposal', color: '0366d6', description: 'Profile sync configuration proposal' },
  { name: 'profile-sync:failure', color: 'd73a4a', description: 'Profile sync run failure' },
  { name: 'profile-sync:improvement', color: 'ffd54f', description: 'Profile sync improvement request' },
