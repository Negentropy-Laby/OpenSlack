...
async function fetchAllIssuesByLabel(
  ...
  while (page <= maxPages) {
    const { data } = await octokit.issues.listForRepo({
      owner, repo, labels: label, state: 'all', per_page: 100, page,
    })
    const issues = (data as unknown as Array<RawIssue & { pull_request?: unknown }>)
...
export async function fetchWorkflowLifecycleIssues(
  ...
  await Promise.all([
    ...
    octokit.pulls.list({ owner, repo, state: 'all', per_page: 100 })
      .then(({ data: prData }) => {
        const prs = prData as unknown as Array<{ number: number; title: string; html_url: string; state: string }>
        for (const pr of prs) {
