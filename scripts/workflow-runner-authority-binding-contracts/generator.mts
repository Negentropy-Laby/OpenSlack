...
const controlSequenceRules = validateControlSequences(
  JSON.parse(await readFile(new URL('../../' + sequenceSource, import.meta.url), 'utf8')),
);
...
function schemaForValue(value: unknown, path: readonly string[] = []): Json {
  ...
  if (
    ...
    if (record.phase === 'control_delivery') {
      ...
      properties.controlKind = controlSequenceSchema(
        controlSequenceRules,
        record.controlKind,
        record.companionSequence,
      );
    }
  }
...
async function goldenVectors() {
    ...
    (Object.keys(controlKindInputs) as ControlDeliveryKind[]).map((kind) => [
      kind,
      {
        ...
        receipt: controlDelivery(
          ...
          workflowControlCompanionSequence(kind),
          'accepted',
          kind === 'event_receipt'
            ? null
  ...
  const previousBudgetSource = {
    ...
    durableReceiptBytes: canonicalWorkflowBudgetAuthorityJson({
      ...
      contractManifestSha256: WORKFLOW_BUDGET_ORIGINAL_MANIFEST_SHA256,
    }),
  };
  ...
  const badControlSequence = cloneJson(
    controlKinds.effect_authorization.receipt,
    'bad control sequence',
  );
  badControlSequence.companionSequence = 3;
  const badDecisionOrdering = cloneJson(
    controlKinds.effect_authorization.receipt,
    'bad decision ordering',
  );
  const badDecisionMessage = cloneJson(
    controlKindMessages.effect_authorization,
    'bad decision revision',
  );
  badDecisionMessage.runRevision =
    exchanges.effect_authorize.stage.runnerAuthority.expectedGlobalRunRevision;
  badDecisionOrdering.messageDigest =
    prepareWorkflowControlAuthorityMessage(badDecisionMessage).messageDigest;
  ...
  return {
    ...
    negative: exactNegatives([
      ...
      negative(
        'control-kind-sequence-conflict',
        'validate_control_delivery',
        {
          receipt: badControlSequence,
          message: controlKindMessages.effect_authorization,
          stage: exchanges.effect_authorize.stage,
          resolution: exchanges.effect_authorize.resolution,
          resolutionReceipt: exchanges.effect_authorize.resolutionReceipt,
...
async function projectedRunnerOpenAPI(map: ReadonlyMap<string, Buffer>): Promise<string> {
  const current = await readFile(
    resolve(repositoryRoot, 'services/workflow-control/docs/api/runner-openapi.yaml'),
    'utf8',
  );
...
export async function generate(): Promise<void> {
  ...
  const openAPI = await projectedRunnerOpenAPI(map);
  for (const [path, bytes] of sequenceFiles) {
  ...
  await mkdir(dirname(runnerOpenAPIPath), { recursive: true });
...
export async function check(): Promise<void> {
  ...
  for (const [path, bytes] of sequenceFiles) {
    if (!(await readFile(resolve(outputRoot, path))).equals(bytes))
      throw new Error('Control sequence rule projection is stale: ' + path);
  }
  const map = await outputs();
