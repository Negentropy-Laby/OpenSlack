    ...
    switch (category) {
      ...
      case 'github-review': {
        return {
          success: false,
          message:
            'GitHub PR approval requires human GitHub identity. Use: gh pr review <PR> --approve',
      ...
      case 'profile-sync': {
        return {
          success: false,
          message:
            'Profile sync approval requires explicit human action. Use CLI: openslack collaboration workflow profile-sync run',
  ...
  try {
    if (mode === 'preview') {
      ...
      return {
        ...
        data: {
          mode: 'preview',
          phases: mod.meta.phases.map((p) => p.title),
...
export async function requestWorkflowReview(
  ...
  try {
    const { findWorkflow, loadWorkflow, TrustStore, resolveTrustLevel } =
      await import('@openslack/workflows');
    const found = await findWorkflow(workflowName);
    if (!found) {
      return { success: false, message: `Workflow "${workflowName}" not found.` };
    ...
    if (result.errors.length > 0) {
      return {
        ...
        data: {
          ...
          commented: result.commentedIssues,
...
export async function splitWorkflowIntoIssues(
  ...
  try {
    ...
    const options =
      Number.isFinite(parentIssue) && (parentIssue ?? 0) > 0
    ...
    return {
      ...
      data: {
        ...
        subIssues: result.subIssues.map((s) => ({
          phase: s.phase,
...
export function createProfileSyncHandlers(root: string) {
  return {
      ...
      try {
        ...
        const checkGroups = [
          ...
          },
          {
            ...
            detail: result.target.markerExists
              ? 'Marker present in target'
    ...
    createProfileSyncFailureIssue: async function profileSyncFailureIssue(
      error: string,
    ): Promise<TuiActionResult> {
      try {
        const { loadProfileSyncConfig, publishProfileSyncFailure } =
        ...
        return {
          success: true,
          message: `Created issue: ${result.url}`,
          data: { issueUrl: result.url },
        };
      } catch (err: unknown) {
...
export function createActionHandlers(
  ...
  return {
    ...
    splitWorkflowIntoIssues: (name, parentIssue) =>
      splitWorkflowIntoIssues(name, parentIssue, root),
    ...
    recordWorkbenchAction: (threadId, card, message) =>
      recordWorkbenchActionFromTui(threadId, card, message, root, actorId),
