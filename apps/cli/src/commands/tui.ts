...
export function tuiCommands(): Command {
  return new Command('tui')
    ...
    .action(async () => {
      try {
        const { renderShellTui, mapDashboardToViewModel, mapStatusToViewModel, mapPrQueueToViewModel, mapWorkflowGalleryToViewModel } = await import('@openslack/tui');
        ...
        try {
          const { discoverJsWorkflows, discoverYamlTemplates } = await import('@openslack/workflows');
