...
| —（实现边界裁决，非宪法修订） | 2026-07-22 | B1–B4 闭合授权 | 所有者选择 MVP 仅支持 `bearer` outbound auth；`HeaderRule.literal/credential_field` 保留。Vendor Registry 对其他 `auth_strategy` fail closed，数据库升级发现既存非 bearer 配置时拒绝迁移，不做静默转换。 | `design/cdd/vendor-registry.md`、`docs/api/openapi.yaml`、`migrations/000003_b3_registry_fixes.up.sql` |
