-- The earlier result union cannot represent the two IB1-03 vendor terminal
-- reasons. Refuse rollback while such immutable history exists.
...
BEGIN
    IF EXISTS (
        ...
        WHERE reason IN ('vendor_rejected', 'vendor_protocol_error')
    ) THEN
        RAISE EXCEPTION '000008 rollback blocked: v2 acknowledgement result history exists';
    END IF;
END
$$;
