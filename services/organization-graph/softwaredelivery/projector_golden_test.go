...
type projectorGoldenFile struct {
	...
	Cases []projectorGoldenCase `json:"cases"`
}

type projectorGoldenCase struct {
	ID                string `json:"id"`
...
func loadProjectorGolden(t *testing.T) projectorGoldenFile {
	t.Helper()
	path := filepath.Join("..", "internal", "contractmirror", "generated", "software-delivery", "v1", "projector-golden-vectors.json")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read golden vectors: %v", err)
	}
	var result projectorGoldenFile
...
func TestGeneratedProjectorGoldenVectors(t *testing.T) {
	...
	if golden.Schema != "openslack.software_delivery_projector_golden_vectors.v1" || golden.Authority != "typescript" {
	...
	if golden.ProjectorID != ProjectorID || golden.SourceSchema != SourceSchema {
		t.Fatalf("golden contract identity mismatch: projector=%q source=%q", golden.ProjectorID, golden.SourceSchema)
	}
	...
	successes, failures, schemaValid, schemaInvalid := 0, 0, 0, 0
		...
		t.Run(testCase.ID, func(t *testing.T) {
			...
			if testCase.ExpectedError != nil {
				...
				if testCase.ExpectedError.Name != "GraphContractError" {
					t.Fatalf("expected error name = %q, want GraphContractError", testCase.ExpectedError.Name)
				}
			...
			if !bytes.Equal(actual, expected) {
				t.Fatalf("projected canonical bytes differ: actual=%d bytes expected=%d bytes", len(actual), len(expected))
			}
			...
			if err != nil {
				t.Fatalf("decode expected canonical bytes: %v", err)
			}
	...
	if successes == 0 || failures == 0 {
		t.Fatalf("golden vectors must cover success and error paths: successes=%d failures=%d", successes, failures)
	}
	if schemaValid == 0 || schemaInvalid == 0 {
		t.Fatalf("golden vectors must cover schema-valid and schema-invalid sources: valid=%d invalid=%d", schemaValid, schemaInvalid)
	}
}
