...
60-minute job timeout. The dispatch must provide `expected_commit`, and both the hosted preflight
and protected job require it to equal `GITHUB_SHA`. A deployment-owned runner configured by absolute path performs the real
