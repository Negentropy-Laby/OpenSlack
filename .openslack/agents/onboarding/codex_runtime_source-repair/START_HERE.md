...
# Workflow Source Repair: Manual Onboarding

Agent `codex_runtime_source-repair` uses runtime `codex`, role developer in engineering,
...
`.openslack/agents/registry/codex_runtime_source-repair.yaml`, and `docs/contributor/github-issues-loop.md`.
The runtime entrypoint is `.openslack/agents/onboarding/codex_runtime_source-repair/codex_automation_prompt.md`; this package is under
`.openslack/agents/onboarding/codex_runtime_source-repair/`. Git is the source of truth.
...
`.openslack.local/agents/codex_runtime_source-repair/identity.yaml` and configure runtime/bot authentication
...
bun run openslack agent bootstrap --agent-id codex_runtime_source-repair
```
...
```bash
bun run openslack agent tick --agent-id codex_runtime_source-repair --source github-issues --issue-number <ISSUE-NUMBER>
```
...
```bash
bun run openslack task checkout --agent-id codex_runtime_source-repair --issue-number <ISSUE-NUMBER>
bun run openslack github claim heartbeat --agent-id codex_runtime_source-repair --issue-number <ISSUE-NUMBER>
```
...
```bash
bun run openslack task sync --agent-id codex_runtime_source-repair --task-id <TASK-ID> --run-id <RUN-ID> --paths <CHANGED-PATHS> --issue-number <ISSUE-NUMBER>
```
