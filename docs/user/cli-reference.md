...
| Claim one exact ready GitHub Issue               | `openslack agent tick --agent-id <id> --source github-issues --issue-number <n>` | Fails closed without selecting another Issue if the target is ineligible.                            |
...
bun run openslack agent tick --agent-id <id> --source github-issues --issue-number <n>  # Claim one exact Issue
...
2. Create the PR with the bot/agent credential path documented in `docs/operations/github-automation.md`.
...
See `docs/operations/keychain-packaging.md` for the artifact contract.
...
See `docs/contributor/plugins/testkit.md` for the G1-G17 matrix and stable finding
codes.
...
| `openslack agent tick --agent-id <id> --source github-issues`                    | Claim the first eligible GitHub task |
| `openslack agent tick --agent-id <id> --source github-issues --issue-number <n>` | Claim one exact GitHub Issue         |
...
`docs/user/guides/aby-integration.md` for setup and smoke-test steps.
...
`default` or `acceptEdits`. See `docs/user/guides/openai-compatible-runtime.md` for the
...
[`design/cdd/workstreams/notification-delivery/README.md`](../../design/cdd/workstreams/notification-delivery/README.md) and
[`docs/user/guides/notification-delivery-operations.md`](guides/notification-delivery-operations.md).
...
occurred, the sealed evidence is invalid, or the report is `FAIL`. The current
workflow runs only after the IB6 import receipt and produces
`G5-POST-IMPORT-QUALIFICATION` evidence for IB7 evaluation. A `PASS` does not
authorize IB7 cutover or claim the superseded long-duration Canary,
`LIVE_VERIFIED`, OpenSlack 0.3.0, production readiness, or destructive
retirement.
...
| `openslack mcp serve --stdio --profile agent-bound --principal-ref <agent-id> --governance-authority-backend go --governance-authority-routing-epoch <n> --governance-authority-tenant <workspace-id> --governance-authority-origin <origin> --governance-authority-build-sha <sha> --governance-authority-caller <caller-id> --governance-authority-expires-at <timestamp>`                | Route only newly created governed plans to the epoch-bound Go/PostgreSQL writer; existing plans keep their writer    |
| `openslack mcp serve --stdio --profile agent-bound --principal-ref <agent-id> --governance-authority-backend ts-local --governance-authority-routing-epoch <higher-n> --governance-authority-tenant <workspace-id> [--governance-authority-origin <origin> --governance-authority-build-sha <sha> --governance-authority-caller <caller-id> --governance-authority-expires-at <timestamp>]` | Route only new plans back to TypeScript; retain the complete Go transport while older Go plans drain                 |
...
`--governance-authority-backend` is the independent GS6 governed-plan writer selector for
new records. Its common policy requires a positive canonical safe-integer routing epoch and an
exact canonical workspace/tenant assertion. `go` additionally requires the complete transport
group: origin, expected 64-lowercase-hex build SHA, caller ID, and bounded expiry. Supplying only
part of that group fails startup and identifies the missing binding. The optional `internal`
network mode still permits only the reviewed private/link-local IP-literal boundary.

The route is immutable per plan. A higher-epoch `ts-local` policy changes only plans created after
that policy starts; it cannot rescue, copy, or reroute an existing Go-routed plan. During rollback,
disable new acceptance in Governance Control, retain the old Go epoch as a drain epoch, and keep
Go, PostgreSQL, and the complete transport available until every known Go plan is terminal or
reconciled and its pending audit delivery is recorded. A Go or PostgreSQL outage therefore blocks
create, confirm, cancel, and transition work for affected Go-routed plan IDs; it never triggers a
local write or per-request fallback.

Governed-plan enumeration is intentionally local-only in GS6. The Go authority exposes bounded
point reads by known `planId`, not an unbounded list route, so local plan lists and dashboards that
use the routed store's `list()` do not include Go-routed records and must not be treated as a total
plan count. Preserve the `planId` from MCP preview/confirmation receipts and correlate it with the
Governance Control point read and Collaboration audit evidence during drain and reconciliation.
...
[`developer/qoder-mcp.md`](../architecture/integrations/qoder-mcp.md).
...
[`developer/module-maturity.md`](../reference/module-maturity.md) for evidence and
compatibility rules.
