...
id: cdd-workstream-scenario-runtime
status: In Review
authority: canonical
audience:
  - contributors
...
Scenario Runtime loads code-free Scenario Packs and compiles bounded intent into
immutable governed plans.

## User Promise

Users can discover, preview, instantiate, persist, and explain scenarios while
keeping mutation and approval outside the pack.

## Data Model

Exact-byte pack, lock, sealed projector/capability catalogs, instance, immutable
plan, effect hash, correlation, approval record, and CAS state.

## Edge Cases

Path escape, symlink/reparse changes, mid-read mutation, unknown capability,
unregistered workflow target, stale plan, or identity mismatch fails closed.

## Dependencies

`@openslack/scenario-runtime`, Organization Graph projectors, and governed
Workflow execution.

## Configuration

Pack discovery is bounded and process-cached. Pack changes require restart.

## Acceptance Criteria

- Packs cannot contain arbitrary commands, module paths, or unattended flags.
- Persisted plans revalidate current sealed bindings.
- Preview and approval do not imply effect execution.
