...
id: cdd-workstream-agent-runtime-and-aby
status: In Review
authority: canonical
audience:
  - contributors
...
This workstream provides the governed agent launcher, provider boundary,
transcript/run evidence, tool permissions, worktree isolation, and Aby bridge.

## User Promise

Users can run configured agents with visible identity, permissions, model,
tools, budget, transcript, and failure evidence.

## Data Model

Agent definition, provider binding, permission profile, worktree, run metadata,
transcript events, tool results, and lifecycle evidence.

## Edge Cases

Missing provider, invalid metadata, unavailable MCP, path escape, secret
exposure, or forbidden authority fails closed.

## Dependencies

`@openslack/agent-runtime`, workspace/runtime isolation, collaboration events,
and an explicitly configured external provider or Aby process.

## Configuration

Provider and credentials are references, never committed values. Aby remains an
external runtime and does not become project governance authority.

## Acceptance Criteria

The acceptance checklist above is design evidence only. Status promotion
requires executable tests and scoped run evidence in the project state.
