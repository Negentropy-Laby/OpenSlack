...
id: cdd-workstream-profile-sync
status: In Review
authority: canonical
audience:
  - contributors
...
For the broader integration framing, see [design/cdd/workstreams/negentropy-integration/README.md](../negentropy-integration/README.md).

## Overview

Profile Sync projects approved source content into a GitHub profile through a
governed branch and pull request workflow.

## User Promise

Users can preview exact changes, avoid duplicate work, and trace failures and
delivery without granting the projection source authority.

## Data Model

Source and target repositories, selected content, normalized projection,
idempotent branch, run evidence, failure Issue, and PRMS status.

## Edge Cases

Unchanged content is idempotent. Concurrent or failed runs deduplicate. Missing
bot identity, ruleset evidence, or source content blocks delivery.

## Dependencies

Workflow runtime, GitHub task/PR infrastructure, and collaboration events.

## Acceptance Criteria

- Source content remains authoritative.
- Delivery is bot-authored and human-reviewed where required.
- Projection failures create bounded, deduplicated evidence.
